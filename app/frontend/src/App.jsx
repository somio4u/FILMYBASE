...
const LABELS = {
  en: {
    ...
    productionModeQuestion: 'Is this a shooting production, or an AI movie?',
    shootingProductionOption: 'Shooting Production (I will shoot this)',
    aiProductionOption: 'AI Movie',
    aiMoviePlaceholderTitle: 'AI Movie',
    aiMoviePlaceholderNote: "This pipeline is being built next — a separate path for a project that's generated by AI rather than physically shot, sharing the same story knowledge but working differently after the screenplay stage.",
  ...
  or: {
    ...
    productionModeQuestion: 'ଏହା ଏକ ସୁଟିଂ ପ୍ରଡକ୍ସନ୍, ନା ଏକ AI ମୁଭି?',
    shootingProductionOption: 'ସୁଟିଂ ପ୍ରଡକ୍ସନ୍ (ମୁଁ ଏହାକୁ ସୁଟ୍ କରିବି)',
    aiProductionOption: 'AI ମୁଭି',
    aiMoviePlaceholderTitle: 'AI ମୁଭି',
    aiMoviePlaceholderNote: 'ଏହି ପାଇପଲାଇନ୍ ପରବର୍ତ୍ତୀ ପର୍ଯ୍ୟାୟରେ ତିଆରି ହେବ — ଏକ ଅଲଗା ପଥ ଯାହା ଶାରୀରିକ ଭାବରେ ସୁଟ୍ ହେବା ପରିବର୍ତ୍ତେ AI ଦ୍ୱାରା ତିଆରି ହେଉଥିବା ପ୍ରୋଜେକ୍ଟ ପାଇଁ, ସମାନ କାହାଣୀ ଜ୍ଞାନ ବ୍ୟବହାର କରି ସ୍କ୍ରିନପ୍ଲେ ପର୍ଯ୍ୟାୟ ପରେ ଭିନ୍ନ ଭାବରେ କାମ କରେ।',
...
function App() {
  ...
  // 'shooting' = today's existing pipeline, completely unchanged. 'ai' = a
  // new, separate pipeline for a project that's never physically shot (no
  // crew, no casting, no shoot schedule) — its own panel, built out next.
  const [productionMode, setProductionMode] = useState('shooting')
      ...
      {!storylines && !(conceptId && projectType === 'story') && (
        <div className="empty-state">
          <div className="format-picker">
            <h4 className="format-picker-title">{t.productionModeQuestion}</h4>
            <div className="format-picker-row">
              <label className="format-radio">
                <input
                  ...
                  value="shooting"
                  checked={productionMode === 'shooting'}
                  onChange={() => setProductionMode('shooting')}
                />
                {t.shootingProductionOption}
              </label>
              <label className="format-radio">
                <input
                  type="radio"
                  name="production-mode"
                  value="ai"
                  checked={productionMode === 'ai'}
                  onChange={() => setProductionMode('ai')}
                />
                {t.aiProductionOption}
              </label>
            </div>
          </div>
          ...
          {productionMode === 'ai' ? (
            <div className="format-picker">
              <h4 className="format-picker-title">{t.aiMoviePlaceholderTitle}</h4>
              <p className="sidebar-section-note">{t.aiMoviePlaceholderNote}</p>
            </div>
          ) : (
          <>
          {startStage === 'idea' && (
