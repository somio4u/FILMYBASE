...
- After a side conversation about spinning up a separate offline/local replica of this whole app (not built — the user didn't want that handled in this session), the user asked for something that actually fits here: the ability to give ONE project its own extra creative direction (tone, genre conventions, character voice notes) without touching how the agents behave for every other project. Built this as an admin-only "Project Instructions" panel in the sidebar — whatever's typed there gets layered on top of the Story & Screenplay agent's existing prompts for that one project, never replacing them, so the JSON structure the rest of the app depends on stays intact. Every other project keeps behaving exactly as before, since a project with nothing typed in gets a byte-identical prompt to what existed before this feature. Wired into the screenplay-writing step first (one call site's data-loading query now also fetches it, so no other code needed to change); the three-act, bit sheet, scene list, and script-breakdown stages aren't wired up yet — same easy pattern, just not done everywhere yet. One thing only the user can do: the live Supabase database needs one new column added by hand (`ALTER TABLE concepts ADD COLUMN IF NOT EXISTS custom_instructions TEXT;`, run once in Supabase's SQL editor) since this sandbox has no way to reach production directly. Verified the merge logic standalone and the whole save/load round-trip live in a browser. Next: extend the same instructions to the other Story & Screenplay stages once the user confirms this first slice actually changes generated content the way they expect; also ran into another Claude session's own commit on `main` mid-session (a plain-language quota-error message) — merged it in cleanly, no conflicts.
