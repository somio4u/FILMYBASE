# Progress Tracker

**How to use this**: Start every Claude Code session by saying:
"Read PROGRESS.md and CLAUDE.md, tell me what's next, and let's continue."

Check off `[x]` as things get done. Claude Code should update this file
itself as work finishes — if it doesn't, ask it to.

---

## App Deployed Online — No Longer Just Local ✅ DONE
User asked to "make it online" so the app can be reached from a browser
instead of only running on one laptop. Checked and confirmed, rather than
re-doing from scratch, since a lot of this turned out to already exist:

- [x] **Database + file storage (Supabase)** — the `FILM ESSENTIALS`
  Supabase project already has every table from `backend/schema.sql`
  created, plus a public `crew-photos` storage bucket for crew/cast photos.
- [x] **Backend (Render)** — a `FILMYBASE` web service on Render, auto-
  deploying from GitHub's `main` branch on every push. Live at
  **https://filmybase.onrender.com**, confirmed healthy via `/api/health`.
  Free tier, so it "sleeps" after inactivity — the first request after a
  quiet spell can take ~50 seconds to wake up.
- [x] **Frontend (Vercel)** — a `filmybase` project on Vercel, also auto-
  deploying from `main`. Live at **https://filmybase.vercel.app**, with
  `VITE_BACKEND_URL` already pointed at the Render backend.
- [x] **Confirmed working for real** — opened the live site, typed a movie
  concept, hit Generate, got real AI-generated storylines back. Frontend
  and backend are genuinely talking to each other in production, not just
  independently "up."

**What this means going forward:** the app is reachable from any browser
now, not just localhost — share `https://filmybase.vercel.app` with anyone
who needs to try it. Any change pushed to the `main` branch on GitHub
auto-deploys to both Vercel and Render within a minute or two, so no manual
redeploy step is needed after future work. Next: keep building/chatting
here as before — new features still land on a working branch first, same
as always, and reach the live site once merged to `main`.

---

## Agents Restructure: Independent Production Management ✅ DONE
User felt Production Management shouldn't live "after Screenplay" as just
another stage in one long list — it should be genuinely independent of the
Story & Screenplay agent, with each agent switchable on its own and
skippable if unwanted. Confirmed the shape with 3 quick questions first
(agent switching = full tabs, screenplay import = paste+AI extraction,
project identity = fully independent project) before restructuring.

- [x] **Sidebar "Stages" list replaced with an "Agents" list.** Two
  entries — Story & Screenplay, Production Management — each an
  accordion header that expands to show ONLY that agent's own stages when
  selected. Clicking a header switches the whole main content area like a
  tab, not just scrolling.
- [x] **History is now per-agent.** Added a `project_type` column on
  `concepts` (`'story'` or `'production'`). The History list filters to
  only the active agent's projects, and "New Idea" relabels to "New
  Production" while on that tab.
- [x] **Production Management now works as a fully standalone project** —
  no Story & Screenplay data required at all. A new `scene_lists.concept_id`
  column lets a 'production'-type project's scene list hang directly off
  its concept, bypassing the whole pitch-deck/character-sheet/three-act/
  bit-sheet chain entirely. (When a Production tab IS viewing a Story
  project's own finished screenplay instead — the normal continue-in-the-
  same-project case from the previous session — nothing changed there,
  same contextual behavior as before.)
- [x] **Import Screenplay**, to start a Production project from scratch:
  paste raw text, or **upload a real file** — Final Draft (.fdx), Scrite
  (.scrite), Word (.docx/.doc), PDF, or plain text/Fountain. One AI call
  (reusing the same extraction used for paste) derives the scene list
  (locations/timing/purpose/turn) and a plain character-name list — no
  archetype/want/need richness needed here, this is for scheduling, not
  creative development. New `POST /api/import-screenplay-for-production`
  (text) and `/file` (upload, via `multer`) endpoints; installed `pdf-parse`,
  `mammoth` (docx), `word-extractor` (legacy .doc), and `fast-xml-parser`
  (.fdx) — each format is converted to plain text by its own small reader,
  then funneled through the one shared AI-extraction function.
  **Scrite's exact JSON schema couldn't be verified offline** — implemented
  a best-effort structured read of the documented shape with a generic
  "collect readable strings" fallback if that shape isn't found, so it
  degrades gracefully rather than failing outright on a schema mismatch.
- [x] **Found and fixed 3 real cross-tab data leaks while testing**: with a
  production-only project loaded, `concepts.storylines` defaults to `[]`
  (not null) so bare `storylines &&` truthiness checks were firing on an
  empty array — showed an empty "Storyline suggestions" header and the
  wrong "type your idea" bubble on the Story tab, and briefly let the
  Screenplay section render a Production project's scene list under the
  wrong agent. Fixed by checking `.length` instead of bare truthiness, and
  gating story-only content on `projectType === 'story'` in addition to
  `activeAgent === 'story'`. Also had to split the persistent input bar's
  whole mode-detection chain by `activeAgent` first — a shared chain length-
  based guard alone wasn't enough once Production got its own revise-mode.
Tested end-to-end live: switched agents and confirmed the main view fully
replaces itself each time; imported a screenplay via paste AND via a real
uploaded PDF, DOCX, and FDX file (each hand-built and round-tripped through
the actual endpoints, not mocked) — all three correctly extracted title,
characters, and scenes; confirmed Story and Production tabs no longer
cross-contaminate after the fixes above.

---

## Script Breakdown before Scheduling ✅ DONE
User wanted an assistant director's-style script breakdown to happen
**between** a finished screenplay and the shoot schedule — not go straight
from screenplay to schedule. This is standard real-world film practice:
before you can schedule shoot days, someone has to read the whole script
and list out everything each department needs.

- [x] **New "Script Breakdown" stage**, sitting between "Production"
  (import screenplay) and "Shoot Schedule" in the Production Management
  agent's own sidebar sub-stage list. New `script_breakdowns` table
  (same pending/approved/changes_requested pattern as every other stage).
- [x] **One AI pass reads the whole script** and sorts everything into 5
  categories: Artist List (cast), Location List (with INT/EXT and scene
  counts), Property List (props), Costume Changes, and Art Department
  Notes — each bilingual (English + Odia).
- [x] **Review/approve/revise cycle**, same pattern as every other stage in
  the app — request changes with a feedback note, or approve to unlock the
  next stage.
- [x] **Download a PDF for each of the 5 categories separately** — a
  "Download PDF" link on every category card, reusing the same PDFKit +
  Odia-font setup as the screenplay PDF export.
- [x] **Shoot Schedule now requires the breakdown to be approved first** —
  the backend rejects `/api/shoot-schedule` until a script_breakdown for
  that scene list is approved, and the UI only shows the schedule section
  once the breakdown is.
- [x] **Tentative shoot start date + "how many days" question**, shown once
  the breakdown is approved, right before the existing character/location
  availability form. Start date defaults to 3 weeks from today; day count
  defaults to 10. The AI is told to fit the schedule within that many days
  where possible, and each shoot day gets stamped with a real calendar date
  (plain date-arithmetic, not AI-generated, so the dates are always exactly
  right).
- [x] **Fixed a date-off-by-one bug found during testing**: the date
  stamping used `new Date(...).toISOString()`, which converts through UTC —
  on a server running in IST (or any timezone ahead of UTC), that silently
  shifted every shoot day back by one calendar day from the date the user
  actually typed in. Fixed by doing the date math in UTC from the start
  (`Date.UTC(...)`) instead of relying on the local-to-UTC conversion.
Tested end-to-end live on a real project ("The Potter Wheel"): generated a
breakdown and confirmed all 5 categories populated with real content from
the script; downloaded and verified all 5 PDFs (English and Odia) actually
open as valid PDF files; approved the breakdown and confirmed the schedule
section only then appears; set a custom start date and day count and
confirmed the generated schedule's Day 1 date exactly matches the date
typed in.

---

## Script Breakdown: Re-analyze & Manual Edit ✅ DONE
The AI won't always catch every character or prop on its first pass. User
asked for a way to double-check and fix any one section without having to
regenerate (and possibly reshuffle) the whole breakdown.

- [x] **"Re-analyze" button on each of the 5 category cards.** Re-reads the
  full script but focuses on just that one category, explicitly told to
  double-check for anything subtle a first pass could miss (brief
  appearances, background mentions, minor items mentioned once) — shown its
  own previous list for that category so it corrects/extends rather than
  starting blind. The other 4 categories are left completely untouched.
  New `POST /api/script-breakdown/:id/reanalyze` endpoint.
- [x] **"Edit" button on each category card**, turning it into an editable
  list: every existing item becomes text fields (both English and Odia side
  by side, so bilingual output stays intact), a "Remove" button per item,
  a "+ Add Item" button to add something the AI missed entirely, and
  Save/Cancel. New `POST /api/script-breakdown/:id/edit` endpoint — takes
  the full corrected breakdown, no AI call involved for this one.
- [x] Both actions insert a new breakdown row rather than overwriting —
  same non-destructive history as every other stage — which also means the
  breakdown goes back to "pending" and needs approving again before the
  Shoot Schedule stage unlocks, exactly like any other content change would.
Tested live on "The Potter Wheel": used Edit to manually add a missed
minor character ("Village Fair Vendor") to the Artist List and saved it;
used Re-analyze on the Property List and confirmed via the API that it came
back as a fresh row tagged "Re-analyzed: props" — with the manually-added
artist from the previous step still intact, confirming re-analyzing one
category doesn't touch the others.

---

## AGENT 4 — Production Management: Shoot Schedule ✅ DONE (Part 1 of 2)
Agent 1 (Story & Screenplay) is functionally complete per its original
numbered plan (all of Steps 1.1–1.9 done), so per CLAUDE.md's build order
the next milestone should have been Agent 2 (Hero) then Agent 3
(Storyboard). User asked to jump straight to Agent 4 (Production
Management) instead. Flagged the skip and the fact that the agent's own
spec assumes data "from the Storyboard Agent (via the Hero Agent)," which
doesn't exist — user chose to proceed anyway, wiring Production Management
directly to Agent 1's existing scene list rather than waiting on Hero/
Storyboard. The Storyboard/Hero handoff can be added later without
reworking this.

Full spec: `agents/production-management-agent.md`. Its 5 instructions were
split into two builds (small steps) — this is Part 1 (Instructions 1, 2, 4):

- [x] **New "Production" stage**, unlocked once the Scene List is approved
  (doesn't need the full screenplay written — scheduling only needs
  locations/INT-EXT/timing, which the scene list already has).
- [x] **Availability form** (Instruction 1: "gather availability") — one row
  per major character (from the Character Sheet) and one row per unique
  location (deduped from the scene list), each with a free-text "available
  dates" field or an "Unknown — estimate for me" checkbox. This is a
  one-time input shown only before the first schedule is generated, not its
  own approvable stage — matches how the format question worked before it
  moved earlier in the flow.
- [x] **AI-generated shoot schedule** (Instruction 2: "build the schedule") —
  a day-by-day plan grouping scenes by shared location first, then by
  character availability, referencing scenes by position (episode+scene
  index) rather than duplicating their content. Same approve/request-changes
  cycle as every other stage. New `shoot_schedules` table, keyed off
  `scene_list_id`.
- [x] **Conflict flagging** (Instruction 4) — folded into the same
  generation call rather than a separate step: a bilingual `conflicts` list
  the AI is told to use "plainly" rather than producing an optimistic
  schedule that papers over a real problem.
- [x] Wired into the full-project loader, Export/Import, the sidebar Stages
  panel (now Idea → Synopsis → Characters → Bit Sheet → Screenplay →
  **Production**), and the persistent input bar's revise-mode chain.
  Revising the Scene List now also clears any existing shoot schedule
  (its scene references would go stale), matching the cascade pattern used
  everywhere else in the app.
Tested end-to-end live in the browser on "The Raid of Redress": filled in
one character's real constraint ("only available weekends in March") and
marked everything else unknown, generated a 7-day schedule, confirmed it
correctly flagged that exact constraint as a real conflict with a clear
explanation (not silently ignored), confirmed scene references resolved
back to the actual scene one-liners correctly, approved it, and confirmed
it survives a full page reload.

**Not yet built (Part 2, next up)**: Instruction 3 — Budget allocation (a
total-budget input, then an AI-proposed category breakdown with reasoning,
same approve/revise pattern). Instruction 5 (signal completion to Hero
Agent) is deferred along with the rest of the Hero Agent handoff.

---

## Colloquial Odia Dialogue + Flashback/ECHOING Format ✅ DONE
User dropped a real, professionally-written Odia episode script into a new
`DEMO SCREENPLAY DRAFT/` folder and asked for two things: dialogue that
sounds like genuine spoken Odia (Chalita Bhasha) instead of formal/AI-sounding
Odia (Sadhu Bhasha), and dialogue that carries real emotion instead of
flat "cut to cut" information-passing. Also asked whether to add two extra
format elements found in that demo — attributed POV flashbacks and an
"ECHOING" modifier — decided together with the user to build both at once.

- [x] Read the demo script directly (its PDF text layer is garbled/non-
  Unicode — had to render pages as images to actually read the Odia). It
  confirmed the target style: short broken lines, trailing ellipses, real
  code-switching ("ରୁମ୍", "ବ୍ୟାଗ୍", "ଅଙ୍କଲ୍" dropped straight into Odia
  script), and verb endings that shift by relationship (respectful ଛନ୍ତି
  toward an elder, intimate ତୁ/ଯିବୁ from child to mother).
- [x] Rewrote `SCREENPLAY_SYSTEM_PROMPT`'s dialogue section with the user's
  own Sanskritized→colloquial word-swap rules (Grahana Karantu→Nia/Dhara,
  Prasthana Kariba→Bahariba/Jiba, Bartalapa→Kathabarta, Krodhita→Ragi) and
  an explicit relationship-based verb-ending rule (respectful ଛନ୍ତି toward
  elders, casual ଛି/ଛୁ/ଛ toward friends/juniors/intimate family — an AI
  default of formal ଛନ୍ତି for everyone was exactly the mistake to fix), plus
  a direct instruction against flat info-passing dialogue.
- [x] Added **POV-attributed flashbacks** — a new `flashback` screenplay
  element (`character` = whose memory, `text` = what's remembered),
  rendered as "FLASH - [CHARACTER]'S POV:" matching the demo's exact
  convention.
- [x] Added **ECHOING** as a `characterModifier` option (alongside the
  already-built CONT'D/O.S./V.O.) for a remembered line replaying in
  another character's mind — typically follows a flashback element.
- [x] **Caught and fixed a real bug during testing**: the AI translated a
  transition marker into Odia ("ଦୃଶ୍ୟ ବଦଳିବ:" instead of "CUT TO:"). Real
  Odia shooting scripts keep these technical markers in English on both
  sides — added an explicit instruction not to translate them.
Tested end-to-end live: wrote a real new screenplay scene and read the
actual generated Odia directly (not just the English side) — confirmed
natural short sentences, real code-switching, and a believable emotional
exchange between a disgraced coach and a temple priest, not stiff textbook
Odia. CONT'D and transition rendering both confirmed working structurally;
flashback/ECHOING are schema-ready and prompted for but weren't triggered
by this particular scene's content (used only when "dramatically
meaningful," by design) — will show up naturally on a scene that calls
for a memory intrusion.

---

## Climax-as-Sequence + Scene-Level Craft ✅ DONE
The last two items from the craft-book analysis (after Characters). Both
additive schema/prompt changes — no new stages, no schema restructuring.

- [x] **Climax is now a 3-beat sequence, not one tag.** Added two new Bit
  Sheet beat types — `crisis` (the protagonist's hardest genuine dilemma,
  connects back to the catalyst) and `realization` (explicit, visible
  self-awareness right after the climax, distinct from `resolution_beat`)
  — both now mandatory-exactly-once anchors alongside the existing 6,
  always in the fixed order Crisis → Climax → Realization near the end of
  Act 3. The prompt also tells the AI to pick a climax SHAPE that fits the
  story's own genre (direct confrontation for action/thriller/sports vs. a
  multi-thread convergence or authority/institution judgment for a family/
  moral/devotional story) instead of forcing one template — this came
  straight from the real screenplays sampled (Neelachala's climax resolves
  morally at a shared festival event, not a fight). Raised the Bit Sheet's
  minimum bit count from 10 to 12 to keep room for the now-8 anchors.
- [x] **Scene one-liners now carry `purpose` and `turn`.** Every scene
  must be tagged `plot_advancing` or `character_revealing` (Field's rule:
  a scene should do one, never both/neither) and given a short `turn`
  phrase naming its value-shift (McKee's scene method — e.g. "pride turns
  to quiet anguish"). Shown as a badge + italic note under each scene.
  This was actually recommendation #3 from the very FIRST craft-book pass
  ("require a stated turn per scene one-liner") — now built too.
- [x] **Screenplay scenes now support CONT'D/O.S./V.O. and transitions.**
  Dialogue elements got a `characterModifier` field (CONT'D if the same
  character keeps talking after a brief interrupting action beat, O.S. if
  heard-not-seen, V.O. for narration/inner-thought/phone-voice); a new
  `transition` element type supports an occasional meaningful "CUT TO:" /
  "MATCH CUT TO:" / "DISSOLVE TO:" line at a scene's end. This closes the
  screenplay-formatting gap flagged since the very first craft-book pass
  (recommendation #5) — CONT'D/O.S./V.O. and transitions are now covered;
  montage blocks and letter-suffixed parallel micro-scenes (11A/11B) are
  NOT — deferred as a bigger structural change, not attempted here.
Tested end-to-end live on the same real project ("The Raid of Redress"):
generated a fresh 3-episode Bit Sheet and confirmed every episode got the
full Crisis → Climax → Realization sequence with genuinely dilemma-shaped
crisis text; generated scene one-liners and confirmed purpose badges and
turn phrases rendered correctly (e.g. "pride turns to quiet anguish"); wrote
one full screenplay scene and confirmed the AI used `BIRA (CONT'D)`
correctly and unprompted on a real interrupted-dialogue beat.

This completes all 3 gaps from the craft-book + real-screenplay analysis
(Characters, Climax-as-sequence, Scene-craft). Smaller/lower-priority items
from that analysis (a "Sequence" grouping between beats and scenes;
Snyder's Logline Test/genre taxonomy; montage blocks) remain unbuilt —
revisit only if the user wants to go further.

---

## New Stage: Create Characters ✅ DONE
User asked for a full analysis of the pipeline against all 5 craft books
(Field, McKee, Snyder, Trottier, Vogler) AND real reference screenplays (K4,
Ashok Kumar, Neelachala Bhakta Nibas, GIRL_PROBLEM) to find missing steps.
Ran 7 parallel research passes (one per book, plus 2 sampling real
screenplay pages). Strong convergent evidence across nearly every source
pointed to 3 gaps: no real Characters layer, climax as one thin beat-tag
instead of a sequence, and no scene-level craft between the one-liner and
full dialogue. Characters was the biggest and most-confirmed, so built it
first (Climax and Scene-craft are next, not started yet).

- [x] **New stage: Create Characters**, inserted right after Pitch Deck
  approval and before Three-Act Structure — deepens the pitch deck's thin
  3-5 "Major Characters" (name/role/emotional-core/conflict) into a full
  character sheet per character: **archetype** (Vogler's 8 — Hero/Mentor/
  Threshold Guardian/Herald/Shapeshifter/Shadow/Ally/Trickster, framed as a
  function that can shift, not a fixed tag, with an `archetypeNote` for
  when it does), **want vs. need** (conscious goal vs. the deeper wound
  actually driving them), **flaw**, **3+ virtues**, **inner conflict**,
  **outer conflict**, **arc** (one-line A→Z change), and an **introduction
  beat** — the specific action that should introduce them on the page,
  matching the real pattern found in every reference screenplay sampled.
  The character playing the Shadow/antagonist also gets a **heroLogline**
  ("their own story, as if they were the hero of it" — Vogler's rule
  against shallow villains). Same approve/request-changes pattern as every
  other stage; keeps the pitch deck's existing names, may add at most one
  extra minor character if the story genuinely needs one.
- [x] **Three-Act Structure generation now requires approved Characters**
  first (was previously gated on the Pitch Deck alone) — enforced on the
  backend, not just hidden in the UI. Character want/need/arc data is now
  passed into the three-act prompt so the acts stay consistent with who
  each character actually is, instead of being generated independently.
- [x] Added a `character_sheets` table (parallel to the other stage tables,
  linked to `pitch_deck_id`). Wired into the full-project loader, the
  Export/Import round-trip, and the sidebar Stages panel (now Idea →
  Synopsis → **Characters** → Bit Sheet → Screenplay). The persistent input
  bar also got a new "revise the characters" mode in its priority chain.
  Extended the skip-to-bitsheet/skip-to-scenelist "skip ahead" backfill
  endpoints to also produce a matching character sheet, for consistency —
  code-reviewed but not live-tested (same quota-conservation call as before).
- [x] **Found a genuinely useful side-effect while testing**: the character
  sheet's `heroLogline` field correctly appeared ONLY on the character
  tagged `shadow` (not on the two `hero`-tagged leads), confirming the
  conditional-field prompt instruction worked exactly as designed.
Tested end-to-end live in the browser on a real project ("The Raid of
Redress"): approved the pitch deck, generated a character sheet (3 seed
characters deepened + 1 new Ally character added, matching the "at most one
extra" rule), approved it, confirmed "Generate Three-Act Structure" only
appeared after that approval (not after the pitch deck alone), and
confirmed the resulting three-act theme was genuinely consistent with the
characters' individual arcs (not generated independently of them).

Not yet built (next up, per the same craft-book analysis): **Climax as a
sequence** (Crisis → Confrontation → Reversal → Realization → Consequence,
instead of one Bit Sheet tag) and **scene-level craft** (plot-advancing vs.
character-revealing check, plus the still-outstanding screenplay formatting
gap — CONT'D, O.S./V.O., montage, transitions — flagged since the very
first craft-book pass and still not built).

---

## Richer Pitch Deck: Major Characters + Elaborated Episode Synopsis ✅ DONE
User felt the pitch deck's episode synopsis (2-3 sentences) wasn't enough to
actually establish a whole episode. Discussed reordering the pipeline
(three-act structure before pitch deck) but recommended against it — it
would mean paying for the most expensive per-episode step for every episode
before the cheap producer-approval gate even happens. Went with enriching
the pitch deck itself instead:

- [x] **Major Characters** — every pitch deck (film or series) now includes
  3-5 major characters: name, a one-line role/descriptor, their emotional
  core (what they secretly want or fear), and their central conflict. Shown
  as its own section on screen (between Target Audience and the episode
  breakdown) and as a new slide in the PDF export.
- [x] **Elaborated episode synopsis** (web series) — grew from a generic
  2-3 sentence plot beat to a real mini dramatic shape: what the episode
  opens on, the conflict that develops through it, and how it turns or
  ends — enough to actually picture the episode, not just guess its topic.
- [x] Extended the same two additions into the three "skip ahead" backfill
  endpoints (skip-to-synopsis/bitsheet/scenelist) for consistency, so a
  pasted-content project's pitch deck matches the new richer shape too.
- [x] **Found and fixed a real, recurring bug while testing this**: Gemini
  occasionally leaks a stray character from a non-Latin script into an
  otherwise-correct ENGLISH sentence — seen twice now (a Hebrew letter, then
  an Odia letter, both mid-word). The existing sanitizer only ever cleaned
  the Odia ("or") field; added the mirror-image filter for the English
  ("en") field, tested against the exact glitches seen plus normal
  punctuation/accented names to make sure nothing legitimate gets stripped.
Tested end-to-end live: generated a real 3-episode series pitch deck,
confirmed 3 well-formed major characters and genuinely richer per-episode
synopses on screen, and confirmed the PDF export (9 pages: cover, premise,
tone/genre, target audience, major characters, 3 episodes, closing) — sent
to the user directly to inspect.

---

## Small Tweaks ✅ DONE
- [x] **Default format numbers changed**: Film default runtime 90 → 120
  minutes; Web Series defaults 12 episodes × 25 min → 10 episodes × 10 min
  each. Updated everywhere these defaults appear (the first-screen format
  picker, and the backend's safety-net fallback values). Verified live.
- [x] **Removed role suffixes from approval buttons/badges** — "Approve as
  Producer" → "Approve", "🔒 Locked as Story Writer" → "🔒 Locked", "Approve
  Bit Sheet (as Story Writer)" → "Approve Bit Sheet", "Approve Scene List
  (as Screenplay Writer)" → "Approve Scene List". User felt the role labels
  added friction while playing every role solo. This reverses a decision
  CLAUDE.md had explicitly documented (see the note added there) — the
  approval STAGES themselves are unchanged, only the on-screen wording.

---

## Format Question Moved to the Very First Screen ✅ DONE
User asked: the first screen should ask "film or web series?" first, then
duration (film) or episode count + minutes/episode (series) — before typing
an idea at all, instead of asking after choosing a storyline like before.

- [x] Added a "Is this a film or a web series?" card at the very top of the
  empty state (above the greeting and the "Start from" tabs), reusing the
  existing radio + duration/episode fields, defaulting to Film / 90 minutes.
  Only shown for the normal Idea flow — the skip-ahead paste flow (Synopsis/
  Bit Sheet/Scene One-Liners) keeps its own dedicated film-only runtime field
  since series isn't supported there yet (see previous section).
- [x] `/api/generate-storylines` now takes the chosen format and threads it
  into the prompt, so storylines are shaped for a film vs. a multi-episode
  series arc from the very first draft, not decided after the fact.
- [x] Removed the now-redundant second format question that used to appear
  after choosing a storyline — since format is already known up front,
  choosing a storyline now goes straight into building the pitch deck (a
  brief "Building your pitch deck…" message shows while it's in flight; on
  failure the choice unlocks so you can try again or pick the other option).
Tested end-to-end live in the browser: picked Web Series then switched back
to Film, typed a new idea, generated two format-aware storylines, chose one,
and confirmed it skipped straight to a correctly-tagged "FEATURE FILM" pitch
deck with no second format question in between.

---

## Export/Import, History Management, Skip-Ahead ✅ DONE
User asked for six things in one go: offline save/load (export/import a
project as a file on your own computer), a way to start directly from the
Synopsis / Bit Sheet / Scene One-Liners stage instead of always from Idea,
having pasted content at any of those stages auto-generate the missing
earlier stages so the rest of the app still works normally, and History
management (delete, pin — rename was already there).

- [x] **Export Idea / Import Idea** — two new buttons under "New Idea".
  Export downloads the current project (concept, storylines, pitch deck,
  three-act structure, bit sheet, scene list, every written screenplay
  scene) as one `.json` file to your computer — pure client-side, no server
  round-trip needed since the data's already in memory. Import reads that
  file back and rebuilds the whole chain as a brand-new project via a new
  `POST /api/concepts/import` endpoint. Verified the full round-trip
  end-to-end (export → import → reload → every stage's status and content
  matched exactly), including the cascade-delete change below.
- [x] **History: pin and delete** (rename already existed). Every project
  row now has its own pin and delete icon, not just the active one. Pinned
  projects sort to the top of History automatically. Deleting asks for
  confirmation and — since the database foreign keys were changed from
  `ON DELETE SET NULL` to `ON DELETE CASCADE` — cleanly removes the entire
  chain (pitch deck, three-act, bit sheet, scene list, every screenplay
  scene), not just the concept row. Verified the cascade directly: deleting
  a concept with a full chain attached left zero orphaned rows behind.
- [x] **Skip ahead / start from a later stage** — a small "Start from: Idea
  / Synopsis / Bit Sheet / Scene One-Liners" tab row above the input on a
  fresh project. Picking anything other than Idea swaps in a paste box (for
  your own already-written synopsis, bit sheet, or scene list text) plus a
  runtime-minutes field and a Continue button.
- [x] **Auto-generating the missing earlier stages** — pasting, say, a Bit
  Sheet doesn't just save that text; one AI call both (a) faithfully
  restructures your pasted text into the app's real Bit Sheet schema
  (bilingual, correct beat types) and (b) invents short, plausible earlier
  stages (concept, storyline, pitch deck, locked three-act structure) that
  are consistent with it, so the normal approve/revise/lock flow keeps
  working from that point on exactly like a project that started from Idea.
  Same pattern for Synopsis (backfills concept+storyline) and Scene
  One-Liners (backfills concept+storyline+pitch deck+three-act+bit sheet).
  **Film format only for now** — a web series needs per-episode consistency
  a single pasted excerpt can't reliably reverse-engineer yet; picking
  "Web Series" isn't offered in this flow.
  **Quota note, worth knowing**: each skip-ahead action is exactly ONE
  Gemini call regardless of how many stages it backfills (all the backfilled
  stages and the parsed target stage come back in one combined structured
  response) — kept deliberately to one call given the 20/day free-tier cap,
  rather than the 3-5 separate calls a naive "generate each stage one by
  one" approach would have cost.
- [x] Tested `skip-to-bitsheet` for real end-to-end: pasted a short bit-sheet
  outline (a fisherman story), got back a fully backfilled concept →
  approved pitch deck → locked three-act → pending bit sheet whose beats
  faithfully matched what was pasted, with the connecting beats (theme
  stated, setbacks, etc.) plausibly invented to complete the required
  structural anchors. The persistent input bar correctly landed in
  "revise the bit sheet" mode immediately, same as a normally-built project.
  Noted one rare AI glitch during this test — a stray Hebrew character
  leaked into one English-language beat title ("Fיית Fury Unleashed"). Very
  unusual (the existing Odia-script sanitizer doesn't cover this since it
  only cleans the "or" field), not fixed as a general filter since it's a
  one-off seen only once across this whole project's testing — regenerating
  that one beat via "Request Changes" clears it. Worth a proper fix later if
  it turns out to recur.
- [x] Verified `skip-to-synopsis` and `skip-to-scenelist` by code review only
  (not run live) to conserve the day's AI quota, since they reuse the exact
  same one-call backfill pattern the bitsheet test already proved works —
  recommend the user try these two themselves when convenient.

Not built: series support for skip-ahead (see note above); rename and
delete's confirmation dialogs use native browser `prompt()`/`confirm()`,
which couldn't be driven by the automated test browser — please try those
two yourself.

---

## UI Redesign — Gemini-Style Chat Layout ✅ DONE
**Note:** an earlier version of this section described a stage-nav sidebar
(Idea/Synopsis/Bit Sheet/Screenplay, click-to-jump). That was built, then the
user reviewed it live and said directly: "you have created a messed up ui."
It was fully replaced by the design below — no stage nav exists anymore.

- [x] User shared a real Google Gemini UI screenshot plus two custom icon
  sets, and specified exactly how it should work: a plain text bubble on
  first load ("type your idea and explore"), a single persistent input bar
  at the bottom, side-by-side storyline options after hitting Enter, an
  empty Enter regenerating both options, and choosing one visually locking
  it (with the other dimmed) before the film/series question appears.
- [x] Removed the stage-nav sidebar entirely. Sidebar is now just: logo +
  title, a "New Idea" button (resets all state), a "History" section
  (placeholder — see below), and the language toggle pinned to the bottom.
- [x] Main content is a real scrolling chat log, not a single active panel.
  Empty state shows a centered gradient greeting; once a concept exists, an
  AI-style bubble appears, followed by whatever's been generated so far,
  stacked in order (storylines → format form → pitch deck → three-act →
  bit sheet → scene list → screenplay), same underlying data as before.
- [x] One persistent, contextual input bar fixed to the bottom of the main
  pane (hidden once a pitch deck exists, since the rest of the flow uses its
  own inline forms/buttons):
  - Before storylines exist: typing an idea + Enter (or the send button)
    generates them.
  - After storylines exist and none is chosen: the same bar becomes a
    "regenerate" box — Enter with it empty regenerates 2 new options;
    typing feedback first and hitting Enter regenerates with that feedback
    folded in.
- [x] Storyline options render side by side ("Option 1" / "Option 2" labels).
  Choosing one shows a "✓ Locked in" badge on it and visually dims the other
  (its "Choose this one" button stays clickable, so switching is possible
  before the format step is submitted).
- [x] Full dark, Gemini-inspired color theme applied app-wide (not just the
  sidebar) — deep near-black background, dark card panels, a blue-to-purple
  accent gradient for the greeting/buttons/badges, and every existing card
  (pitch deck, three-act structure, bit sheet, scene list, screenplay) reskinned
  to match instead of staying light-themed. Removed a leftover Vite-starter
  default in `index.css` (`#root` had a fixed 1126px boxed width with side
  borders and centered text) that was fighting the full-bleed sidebar+chat
  layout.
- [ ] Not yet built: the right-side panel (New Artifact / Customize / More
  Filmmaking / Filmmaking Projects) — user's list had a likely duplicate
  entry ("Filmmaking Projects" appeared twice), flagged but not resolved yet.
- [x] Real multi-project switching — see "Multi-Project Support" section below.
  (Was a placeholder when this section was first written; now built.)

Tested end-to-end live in the browser: fresh empty state → typed a new idea
→ Enter generated 2 real side-by-side storyline options → choosing one showed
the locked badge and dimmed the other → the film/series format question
appeared right after, exactly matching the user's spec. Also confirmed the
dark theme renders correctly on an existing, far-progressed real project
(pitch deck through screenplay, all reskinned and readable).

---

## Multi-Project Support + Always-On Input Bar + Stage Panel ✅ DONE
User reported four problems after using the Gemini-style rebuild for real:
projects kept mixing up ("why does it always show the last project"), there
was no way to explicitly load an old project or start a genuinely fresh one,
the input bar disappeared after the synopsis step instead of staying put like
a real chat, and the stage-progress list from the earlier (reverted) sidebar
was missed. All four turned out to share one root cause and got fixed together.

**Root cause found:** every `/latest` backend endpoint (pitch-deck, three-act,
bit-sheet, scene-list) just grabbed the single newest row in its whole table,
with no idea which project/concept it belonged to. Loading the app always
reattached whatever was newest ANYWHERE, not "your current project" — so two
different projects touched close together in time could get their content
crossed.

- [x] **Backend**: added a `title` column to `concepts` (for renaming), plus
  three new scoped endpoints — `GET /api/concepts` (list every project for
  History), `POST /api/concepts/:id/title` (rename), and
  `GET /api/concepts/:id/full` (loads ONE project's entire chain — pitch deck
  → three-act → bit sheet → scene list — by walking the real foreign-key
  chain, never "whatever's newest").
- [x] **Load / New / History**: the sidebar's History section now lists every
  real project (newest first), each clickable to load that exact project's
  full state via the new scoped endpoint. "New Idea" now also clears a
  `localStorage` pointer, so a page refresh after starting fresh actually
  stays fresh instead of silently reloading old work — this was the literal
  "why does a new project not start fresh" bug. No separate "Save" button was
  needed since every step already auto-saves to the database the instant it's
  generated; instead, added a rename option (pencil icon next to the active
  project, prompts for a name) so projects are easier to tell apart in History
  than just raw concept text.
- [x] **Stage progress panel, brought back** — a small clickable Idea /
  Synopsis / Bit Sheet / Screenplay list in the sidebar showing done (green) /
  current (blue) / upcoming (grey) for the loaded project. Unlike the earlier
  reverted stage-nav, clicking a stage does NOT hide/replace the chat log —
  it just smooth-scrolls to that section, since the "single stage replaces
  everything" model was the actual thing the user disliked before, not the
  concept of a progress list.
- [x] **Persistent input bar, now truly persistent** — it no longer disappears
  once a pitch deck exists. It now stays mounted for the entire project and
  changes what it does based on what's currently awaiting a decision: type an
  idea (nothing generated yet) → generate storylines; empty Enter or feedback
  (storylines shown, none chosen) → regenerate; typed feedback whenever a
  pitch deck / three-act structure / bit sheet / scene list exists and isn't
  yet approved/locked → submits as a revision to THAT stage (reusing the same
  non-destructive request-changes endpoints the inline "Request Changes"
  boxes already use, just triggered from one place). When nothing is
  currently revisable (e.g. between approving one stage and generating the
  next), the bar stays visible but shows an inert "nothing to revise right
  now" placeholder instead of silently doing nothing.
- [x] **Sidebar layout fix**: the sidebar was scrolling away together with
  the main chat log instead of staying put, because `.app-shell` used
  `min-height: 100vh` (lets the whole page grow taller than the screen) instead
  of a fixed `height: 100vh` with its own internal scroll areas. Fixed so the
  sidebar and the main chat area now scroll fully independently — the sidebar
  never moves.
- [x] Cleaned up 4 abandoned test rows a concurrent test session created by
  pasting bit-sheet content into the "type your idea" box while the app was
  in a confusing intermediate state (from a bug in the very first draft of
  the mode-detection logic above, fixed before this was finished) — deleted
  since nothing downstream referenced them.

Tested end-to-end live in the browser: loaded a real far-progressed project
from History and confirmed the Stages panel correctly showed Idea/Synopsis/
Bit Sheet done and Screenplay current; confirmed the persistent bar correctly
detected "nothing to revise" for that project's current state instead of
wrongly offering to regenerate storylines (a real bug caught and fixed during
testing — loading a project loses the in-memory "which storyline was chosen"
marker, so the very first version of the mode-detection incorrectly fell back
to "regenerate" mode even when a pitch deck already existed); confirmed
clicking a stage jumps the chat log without hiding anything; confirmed
"New Idea" followed by a real page refresh stays on a blank project instead
of reloading old work; confirmed the sidebar stays fixed while scrolling
through a very long, fully-drafted screenplay. Rename was implemented and
code-reviewed but needs the user's own manual test, since it uses a native
browser prompt() dialog that the automated browser tool can't drive.

---

## Screenwriting Craft Guides (analysis done — not yet implemented)
- [x] User added 5 industry-standard screenwriting craft books into
  `screenwriting-craft-guides/` (renamed/cleaned up from a messier folder):
  Syd Field's *Screenplay*, Robert McKee's *Story*, Blake Snyder's
  *Save the Cat!*, David Trottier's *The Screenwriter's Bible*, and
  Christopher Vogler's *The Writer's Journey*.
- [x] Read all 5 and compared each author's framework against our actual
  pipeline (Concept → Storylines → Pitch Deck → Three-Act Structure →
  Bit Sheet → Scene One-Liners → Screenplay). Full findings summarized in
  chat; not duplicated here to avoid drift — ask Claude to re-summarize if
  needed, or check memory.
Five concrete recommendations came out of this. User said "build all one by
one" — building sequentially in this order, each tested before the next:

1. [x] **DONE** — Expand the Bit Sheet's beat-type list with the missing
   structural anchors (from Field + Snyder). `BIT_BEAT_TYPES` grew from 7 to
   13: added `opening_image`, `theme_stated`, `plot_point_1`, `all_is_lost`,
   `plot_point_2`, `final_image` as mandatory-exactly-once anchors (the
   original 7 stay flexible/repeatable). The prompt now tells the AI exactly
   where each anchor belongs (opening_image = first bit, plot_point_1 = last
   bit of Act 1, etc.). Raised the minimum bit count from 6 to 10 so there's
   room for both the anchors and the flexible connective beats. Added
   frontend labels + a distinct badge color per new type (all_is_lost gets a
   stark black badge, matching Snyder's "false death" framing). For a
   series, each episode gets its own complete set of anchors (its own
   opening/final image, its own two plot points), not just once for the
   whole series — each episode is still a self-contained mini-story.
   Tested: regenerated bit sheets for both an existing film and an existing
   4-episode series project — anchors landed in exactly the right positions
   every time (confirmed programmatically: `plot_point_1` was always the
   last Act 1 bit, `final_image` always the last bit overall, etc.), the
   `theme_stated` beat genuinely stated a thematic argument through a
   character's remark, `opening_image`/`final_image` genuinely mirrored
   each other (eve preparation → dawn celebration, showing the change),
   downstream scene-list generation still worked correctly from the richer
   bit sheet, and the Odia-script sanitizer still found zero issues.
2. [ ] Add an explicit Theme/Controlling Idea field, threaded into later
   generation prompts — from McKee + Snyder. **Next up.**
3. [ ] Add a required "turn"/value-shift to each scene one-liner so flat,
   non-dramatic scenes get caught early — from McKee.
4. [ ] Add a structured Characters layer (name, archetype/role, one-line
   arc) referenced by ID from bits and dialogue, instead of character
   only existing as a free-text name string — from Vogler. Likely the
   single biggest/most valuable gap found.
5. [ ] Expand the screenplay element schema with missing industry-standard
   formatting (CONT'D, O.S./V.O., montage, transitions, intercut,
   flashback wrappers) plus a page-count-based duration cross-check
   alongside the current AI-guessed minutes — from Trottier.

---

## Reference Screenplays (writing-style improvement — not a numbered step)
- [x] Folder created: `reference-screenplays/` with `odia/film/hits/`,
  `odia/film/average/`, `odia/series/`, `odia/telefilm/`, and
  `other-languages/` subfolders, plus a README explaining how to use it.
  The hits-vs-average split is deliberate — comparing a hit against an
  average film side by side should surface what actually makes the
  difference (pacing, dialogue, hook), not just "what Odia looks like."
- [x] User added real screenplays into `odia/series/` (K4 episodes 1-6, RITUKHA,
  4TO9, night angel, GIRL_PROBLEM, KP_E_1&2, EP_3_SHOOTING_SCRIPT, a `sasubahu`
  .scrite project, and the 6-episode "ashok kumar" series). Read a
  representative sample across several of these (a crime thriller, a
  suicide-helpline drama, two family dramas, and a youth/social-issue
  drama) — no hits/average or telefilm/film examples yet, only series so far.
- [x] Updated `SCREENPLAY_SYSTEM_PROMPT` in `app/backend/server.js` based on
  real patterns found: dialogue is short/broken/imperfect rather than full
  grammatical sentences, natural English code-switching is common (varies by
  character — urban/young characters code-switch more, rural/older ones
  less), speech register must vary sharply by character type, and family/
  relational address terms (Ma, Bapa, Bhai, Kaka...) get used constantly.
  Caught and fixed a real regression while testing this: because several of
  the real references write dialogue in Romanized Odia (Latin letters) for
  on-set convenience, the AI's first attempt wrote the "or" field in
  Romanized Odia too — breaking the actual bilingual requirement (English +
  genuine Odia SCRIPT). Fixed by explicitly telling the prompt that code-
  switching means an English word embedded inside an Odia-script sentence,
  not writing whole sentences in Latin letters. Verified the fix: regenerated
  the same scene and confirmed the Odia field stayed in real Odia script,
  including the code-switched word itself ("seriously" → "ସିରିଅସ୍ଲି") being
  transliterated INTO Odia script rather than left in Latin.
- [ ] Only `odia/series/` has content so far — `odia/film/hits`, `odia/film/average`,
  `odia/telefilm`, and `other-languages` are all still empty. Revisit this
  pass once (or if) those get filled in, since film/telefilm pacing and a
  genuine hit-vs-average comparison haven't been analyzed yet.

---

## AGENT 1 — Story & Screenplay Agent (build this alone first, no orchestration yet)
Full spec: `agents/story-screenplay-agent.md`

### Step 1.1 — Project skeleton ✅ DONE
- [x] Set up a basic React + Node.js project structure
- [x] Confirm you can run it locally and see a blank page in your browser
- [x] Learning goal: understand what "running the app locally" means

**What was built:** Created `/app/frontend` (React, via Vite) and `/app/backend`
(Node.js + Express). Frontend runs on http://localhost:5173 and shows a simple
placeholder page. Backend runs on http://localhost:4000 and responds at
`/api/health`. Installed Node.js on this machine as part of this step.

### Step 1.2 — Concept input ✅ DONE
- [x] Build a simple page: a text box where you type a movie concept, and a button labeled "Generate"
- [x] Learning goal: understand what a "component" is and how a button triggers an action

**What was built:** `App.jsx` now has a text box for your movie concept and a
"Generate" button. Clicking it currently just displays what you typed below
the button (no AI call yet — that's Step 1.3). Tested in the browser: typed a
sample concept, clicked Generate, confirmed it displayed correctly.

### Step 1.3 — Connect to an AI API using the agent's system prompt ✅ DONE
- [x] Set up an API key for an AI provider (switched to Google Gemini — see note below)
- [x] Use the contents of `agents/story-screenplay-agent.md` as the system prompt for this agent's API calls
- [x] Wire the "Generate" button to send your concept to the AI API and get back storyline suggestions
- [x] Display the 2-3 generated storylines on screen
- [x] Learning goal: understand what an "API call" is, and how a system prompt shapes an agent's behavior

**Stubbed for now (decided 2026-08-11):** No Anthropic API key yet, so the
backend's `/api/generate-storylines` route returns 3 hardcoded fake
storylines instead of calling the real Claude API. This still proves out the
real pattern — frontend calls backend, backend responds, screen updates —
which is the actual learning goal for this step. Tested in the browser:
typed a concept, clicked Generate, got 3 storylines displayed.
**Update 2026-08-11 (switched to Google Gemini):** The Anthropic Claude API
account had no billing credit, so we switched AI providers to Google's
Gemini API instead of adding credit — Gemini's free tier works without
adding payment info. `app/backend/server.js` now calls Gemini
(`@google/genai` package, model `gemini-flash-latest`) with the same system
prompt idea adapted from `agents/story-screenplay-agent.md`, returning
structured JSON. API key is saved in `app/backend/.env` (not committed).
**This is a tech stack change — see the updated "AI" line in CLAUDE.md.**
Tested successfully end-to-end in the browser: typed a concept, got 3 real
AI-generated storylines with title/logline/summary. Step 1.3 is now fully
complete (previously stubbed with fake data — no longer needed).

### Step 1.4 — Save to a database ✅ DONE
- [x] Set up PostgreSQL (locally first, don't worry about cloud yet)
- [x] Save each generated concept + its storylines so they don't disappear on refresh
- [x] Learning goal: understand the difference between "in the browser" (temporary) and "in a database" (saved)

**What was built:** Installed PostgreSQL via Postgres.app (a simple Mac app,
no command line install needed) and created a `filmmaking_app` database with
one table, `concepts` (see `app/backend/schema.sql`). Every time "Generate"
is clicked, the concept + its storylines are saved to this table. The
frontend now also loads the most recently saved concept when the page first
opens (`GET /api/concepts/latest`), so refreshing — or even quitting and
reopening the browser — no longer loses your last result. Tested: generated
a concept, refreshed the page twice, confirmed it reappeared both times.

### Step 1.5 — Pitch deck view ✅ DONE
- [x] Build a simple formatted view of a chosen storyline that looks presentable (title, logline, structure)
- [x] Add a basic export-to-PDF or export-to-doc button
- [x] Learning goal: understand how to generate a downloadable file from app data

**What was built:** Each storyline card now has a "Choose this one" button.
Clicking it sends that storyline to Gemini with a new prompt (adapted from
Instruction 2 in `agents/story-screenplay-agent.md`) asking it to expand the
storyline into a producer-ready pitch: premise, tone/genre, and target
audience. This gets saved to a new `pitch_decks` table and displayed in a
clean, boxed layout below the storylines. An "Export as PDF" button (using
the `pdfkit` package) generates a real, downloadable one-page PDF with the
same content, formatted for presentation.

**Update 2026-08-11 (cultural authenticity + bilingual):** After testing,
feedback was that storylines felt generic/Hollywood-style, not Indian or
Odia. Rewrote the AI prompts (and `agents/story-screenplay-agent.md`) so the
Story & Screenplay Agent now specializes in Odia (Odisha) cinema by
default — Ollywood sensibility, Odisha settings, festivals like Nuakhai and
Rath Yatra, authentic family/social dynamics — instead of generic Hollywood
plots. Every storyline and pitch deck is now generated in **both English
and Odia (Odia script)** simultaneously, and the app has a language toggle
(English / ଓଡ଼ିଆ) at the top that instantly switches everything — page text,
storylines, pitch deck, and even the exported PDF (which required embedding
a real Odia font, Noto Sans Oriya, since the default PDF fonts can't render
Indic scripts). Tested end-to-end in both languages, including opening both
PDF exports and visually confirming the Odia script rendered correctly (not
boxes/garbage).

**Update 2026-08-11 (format selector + presentation-style PDF):** Feedback
was that the pitch deck felt too plain (white page, black text) and lacked
key production info. Two changes:
1. Added a **format selector** — before building a pitch deck, the app now
   asks "Film or Web Series?", and for a series, how many episodes and how
   many minutes each. This is saved with the pitch deck and shown as a badge
   ("FEATURE FILM" or "WEB SERIES · 12 EPISODES × 25 MIN EACH").
2. Rebuilt the PDF export as a real multi-slide presentation deck: landscape
   orientation, 3 slides (cover with title/logline/format tag, premise,
   tone+audience side-by-side), a dark color theme that's picked automatically
   based on the story's genre (e.g. warm amber for family drama, red for
   thriller, teal as a neutral default), decorative accent lines, and an
   elegant serif display font (Playfair Display for English, a bold Noto
   Sans Oriya for Odia — both had to be converted from variable-font format
   to static instances, since the PDF library can't read variable fonts).
   Tested with a Film and a Web Series (15 episodes × 10 min, matching the
   exact numbers given), in both languages, confirming the color theme
   genuinely changes with genre and both scripts render correctly.

**Update 2026-08-11 (matched a real reference deck):** User shared a real
professional pitch deck (for an Odia web series, "HANU-MAN", produced for
Tarang Plus) as the quality bar. Analyzed its structure: landscape 16:9,
cinematic AI-generated poster art, bold condensed red section titles,
split light/dark panel layouts, and — importantly — **one page per episode**
with a synopsis. Checked whether we could generate real AI artwork (Gemini
does have image-generation models) but hit a hard wall: Google's free tier
allows zero image-generation requests — would need billing enabled. User
chose to stay free for now and upgrade everything else. Changes made:
- Added a bold condensed display font (Anton) for section titles/episode
  titles/closing slide, matching the reference's punchy red-title look.
- Rebuilt slides to follow the reference's "light band + big title +
  dark body panel" rhythm (Premise, Tone/Genre, and Target Audience are
  now each their own slide, not combined).
- **New: episode-by-episode breakdown for web series.** When format is
  "series", the app now asks Gemini for a full episode-by-episode
  synopsis (one Gemini call, exact episode count requested) and renders
  one PDF slide per episode — a colored block with a big episode number
  (standing in for the missing photo) next to the title and synopsis.
  Also shown in the on-screen pitch deck view, not just the PDF.
- Added a closing "Thank You" slide, matching the reference's ending.
- Fixed a real bug found during testing: the AI sometimes wrote "Episode
  1:" inside the episode title itself, doubling up with the app's own
  numbering ("Episode 1: Episode 1: ...") — fixed via an explicit prompt
  instruction, verified with a fresh generation.
Tested a 5-episode and a 3-episode series end-to-end (generation + PDF +
on-screen view, both languages) plus re-verified the film path still works
correctly (no episode slides, 5-page PDF).

### Step 1.6 — Approval flow (confirm "as Producer") ✅ DONE
- [x] Add an "Approve" and "Request Changes" button on the pitch view
- [x] If "Request Changes" — add a text box for feedback, and a way to regenerate based on that feedback
- [x] Store the approval status against the project in the database
- [x] Learning goal: understand how apps track "state" (approved vs. pending vs. needs changes)

**What was built:** Added `status` ('pending' / 'approved' / 'changes_requested')
and `feedback` columns to `pitch_decks`. Two new endpoints:
`POST /api/pitch-deck/:id/approve` (marks it approved) and
`POST /api/pitch-deck/:id/request-changes` (records the feedback on the old
version, then generates a brand-new version that addresses it — the old
version is never overwritten, just marked). The UI shows "Approve as
Producer" and "Request Changes" buttons; requesting changes reveals a
feedback box, and after regenerating, the new draft shows a note like
'Revised after feedback: "..."' so it's clear why it changed. Once approved,
the buttons are replaced with a green "✅ Approved as Producer" badge.
Tested end-to-end in the browser: requested changes ("add more suspense,
less comedic") on a lighthearted draft and got back a genuinely tenser
version (new episode titles like "Betrayal and Panic", "Defending the
Sanctuary"), then approved it and confirmed the badge survives a page
refresh (proving it's really saved in the database, not just in-memory).

### Step 1.7 — Three-act structure generation ✅ DONE
- [x] Once approved, add a button "Generate Three-Act Structure"
- [x] Send the approved concept to Claude API, ask for a three-act breakdown, display it
- [x] Learning goal: reusing the same API-call pattern you learned in Step 1.3 — notice it's the same idea again

**What was built:** A new `three_act_structures` table, and a
"Generate Three-Act Structure" button that only appears once a pitch deck
is approved. It sends the approved title/logline/premise/tone to Gemini
(same call pattern as Step 1.3 and the pitch deck) and gets back Setup /
Confrontation / Resolution, each with a summary and a bulleted list of key
beats, fully bilingual. Displayed as its own card below the pitch deck.
Tested end-to-end: generated a real three-act structure from an approved
pitch deck, confirmed it's genuinely tied to that story (character names,
setting, plot beats all match), confirmed it survives a page refresh (saved
in the database), and confirmed the English/Odia toggle works for it too.
Note: this step is generate-and-display only — revision and locking come
next in Step 1.8, on purpose (small steps, one thing at a time).

**Update 2026-08-12 (per-episode three-act breakdown for web series):**
For a web series, the three-act structure now covers two levels at once:
one overall three-act structure for the whole series arc (as before), PLUS
a compact three-act mini-structure for each individual episode (its own
Setup/Confrontation/Resolution, tied to that episode's synopsis from the
pitch deck). Shown as a new "Episode-by-Episode Three-Act Breakdown"
section under the overall structure, one card per episode. This uses the
exact same revision/lock/version-history system from Step 1.8 — no new
buttons needed, regenerating or locking updates both levels together.
Tested end-to-end with a real 3-episode series: confirmed each episode's
breakdown is genuinely distinct and specific to that episode (not repeated
across episodes), confirmed requesting changes ("Episode 2 needs more
tension") correctly escalated that one episode's confrontation beat while
regenerating everything, and confirmed the old version's original episode
breakdowns stayed intact and viewable in Version History. Also confirmed
in both languages. Film format is unaffected (no episode section shown).
Note: for series with many episodes (worked cleanly at 3; not yet tested
at 12-15), the AI response could theoretically get cut off since there's a
lot more content to generate at once — something to watch for if a very
long series is tried and the three-act structure comes back incomplete.

### Step 1.8 — Revision + lock (confirm "as Story Writer") ✅ DONE
- [x] Add a revision loop for the three-act structure (same pattern as Step 1.6)
- [x] Add a "Lock Structure" button — once locked, no more edits (unless explicitly unlocked)
- [x] Keep version history — every past version should still be viewable, not deleted
- [x] Learning goal: understand version history / not overwriting data

**What was built:** Added `status` ('pending' / 'changes_requested' / 'locked')
and `feedback` columns to `three_act_structures`, mirroring Step 1.6's pattern.
New endpoints: `POST /api/three-act-structure/:id/request-changes` (records
feedback on the old version, generates a brand-new revised version — old
version kept, never overwritten) and `POST /api/three-act-structure/:id/lock`
(marks a version locked, after which the UI hides the edit buttons and shows
a "🔒 Locked as Story Writer" badge instead). A `GET
/api/three-act-structure/history` endpoint lists every version for the
current pitch deck, and the UI shows a "Version History" section listing
each one with its status and any feedback given, plus a "View"/"Hide" button
per version that expands to show that exact version's full Setup/
Confrontation/Resolution content (fetched fresh from the database, not just
cached in memory) without touching the current locked version on screen.
Tested end-to-end in the browser: requested changes ("the resolution feels
rushed — add a beat where Dibakar almost fails before succeeding") and got
back a genuinely revised Act 3 with a new near-failure beat added, locked
that version and confirmed the lock badge replaced the edit buttons and
survived a page refresh, then expanded Version 1 in the history list and
confirmed it shows the original, un-revised resolution — proving old
versions are truly preserved and not silently replaced.

### Step 1.8b — Bit Sheet / Plot Points (new stage) ✅ DONE
- [x] Insert a new stage between the locked three-act structure and scene
  one-liners: a Bit Sheet, a more granular list of the story's major
  plot-point beats (catalyst, reveal, midpoint, setback, climax, turning
  point, resolution beat), grouped by act.
- [x] Give it its own Approve / Request Changes cycle (same non-destructive
  revision pattern as everything else), gated on the three-act structure
  being locked first.
- [x] Change scene one-liner generation to build from the approved Bit
  Sheet instead of straight from the three-act structure — each bit
  typically becomes 1-3 scenes.

**Why this got added:** the user shared a much bigger blueprint proposing a
full chat-style UI rewrite, a Bit Sheet stage, an automated quality-scoring
engine, and natural-language stage-jumping — all bundled as one request.
Rather than build all of that at once (which would have broken the
small-steps approach this whole project has followed), the plan was split
into separate pieces and the user picked the Bit Sheet as the lowest-risk,
most-consistent-with-existing-architecture piece to build first. The other
three pieces (chat UI, scoring engine, stage-jump routing) are intentionally
NOT built yet — see the note in the Session Log for what each would involve
if picked up later.

**What was built:** A new `bit_sheets` table (tied to a locked three-act
structure, same status/feedback columns as everything else) and a new
`bit_sheet_id` column on `scene_lists` (replacing the direct link to
`three_act_structures` — scene generation now reads from the Bit Sheet).
New endpoints: `POST /api/bit-sheet` (generate, refuses if the three-act
structure isn't locked), `GET /api/bit-sheet/latest`, `POST /api/bit-sheet/:id/approve`,
`POST /api/bit-sheet/:id/request-changes`. For a web series, each episode
gets its own Bit Sheet (reusing that episode's three-act mini-structure);
for a film, one Bit Sheet for the whole story. Bit count scales loosely
with runtime (roughly one bit per 8 minutes, clamped to a sane range) —
advisory, not a hard rule. The UI shows each bit with a small color-coded
badge for its beat type, grouped by act, with the same Approve/Request
Changes pattern used everywhere else.
Tested end-to-end via the backend and confirmed live in the browser: a
10-bit film Bit Sheet and a 4-episode series Bit Sheet (6 bits/episode)
both generated cleanly, scenes generated correctly FROM the bit sheet
(confirmed a scene list still landed within a few minutes of its runtime
target), a full screenplay scene still generated correctly through the
extended chain, and a revision ("add a bit where he hesitates before
stepping into the pond") correctly inserted a new "Hesitation at the
Water's Edge" beat in exactly the right spot.
Note: this changes the required pipeline order going forward — a scene
list can no longer be generated directly from a three-act structure, it
needs an approved Bit Sheet in between. Older test projects created before
this feature (e.g., the "Shadow of Puri" series scene list mentioned in the
Step 1.9 notes below) needed a fresh Bit Sheet generated before new scenes
could be added to them; nothing already-written was lost, but the
short-cut path no longer exists for old data.

### Step 1.9 — Scene one-liners and full screenplay ✅ DONE
- [x] Add format selector (film vs. web series) if not already asked — done early, in Step 1.5 (see above)
- [x] Generate scene-by-scene one-liners from the locked structure
- [x] Generate full screenplay draft, scene by scene (confirm "as Screenplay Writer")
- [x] Learning goal: chaining multiple agent calls together in sequence within one agent

**What was built (scene one-liners half):** A new `scene_lists` table, tied
to a specific locked three-act structure. Once a structure is locked, a
"Generate Scene One-Liners" button appears (enforced on the backend too —
it refuses if the structure isn't locked yet, matching the agent spec's
"Once locked..." instruction). For a film, it breaks the whole three-act
structure into one ordered scene list; for a web series, it instead breaks
down each episode's own mini three-act structure into that episode's own
scene list, reusing the per-episode breakdown from the earlier update. Each
scene has an act number, a scene heading (interior/exterior, a bilingual
location name, day/night), and a bilingual one-line description — grouped
on screen by act, with act headers. A simple Approve / Request Changes step
(as Screenplay Writer) gates this before the next half (full screenplay) is
built, matching the "never skip an approval step" rule in the agent spec.
Tested end-to-end with the 3-episode series: generated real per-episode
scene lists tied to specific locations and events already established in
the story, requested changes ("Episode 3 needs one more scene showing
Ananya thanking her guru") and confirmed the new scene was inserted in the
right place, approved it, and confirmed everything survived a full page
reload in both languages. Also confirmed the film path (one flat scene
list, no episode grouping) works via a direct backend test.
Note: same AI-generation caveat as the per-episode three-act update — a
couple of stray non-Odia characters occasionally show up inside otherwise-
correct Odia sentences (e.g. a Devanagari word mixed into an Odia one).

**Update 2026-08-12 (Odia script cleanup filter):** Added an automatic
cleanup step, applied to every piece of AI-generated Odia text right after
it comes back from Gemini (storylines, pitch decks, three-act structures,
scene lists — all of it), that strips out any stray characters from other
scripts (Devanagari, Arabic, Bengali, Gurmukhi, Gujarati, Tamil, Telugu,
Kannada, Malayalam) that occasionally slip into an otherwise-correct Odia
sentence. Caught a real near-miss while building this: the first version
of the filter also deleted the Odia sentence-ending punctuation mark "।"
(danda), because that character technically lives inside the Devanagari
Unicode block even though it's the normal full stop in Odia too — every
sentence would have lost its full stop. Fixed by carving out an exception
for that punctuation specifically. Tested with the exact glitchy sentences
seen earlier (confirmed they now clean up correctly) and with a fresh full
three-act regeneration afterward (scanned every single bilingual field
programmatically — zero stray foreign characters found, and all Odia
punctuation intact).

**Update 2026-08-12 (real runtime instead of a fixed scene count):** After
testing, feedback was that a 20-minute episode only got 5 scenes — nowhere
near enough. The root problem: scene generation never actually knew how
long the episode or film was meant to run, and a film had no runtime field
at all (only a web series asked for episode length). Fixed properly:
- Film format now asks for a **Total Runtime (minutes)** when you pick
  "Film," the same way a web series already asks for episode count and
  length. That number is saved with the pitch deck and now drives everything
  downstream.
- Every scene the AI generates now gets its own **estimated duration in
  minutes**, shown right on screen (e.g. "Scene 3 — INT. Tea Stall — DAY
  (~2 min)"), and the app shows a running total against the target ("Estimated
  total: 112 min (target: 120 min)") so it's visible at a glance whether the
  scene list actually covers the runtime.
- The AI is now told the real target runtime and asked to size the scene
  count and each scene's length to genuinely fill it, instead of a vague
  "roughly 4-8 scenes" guess. It also scales complexity: a 12-minute short
  film gets a lean 1-2 beats per act in the three-act structure, while a
  120-minute feature gets 4-6 beats per act with real subplots — same idea
  applied to a web series' overall arc and each episode individually.
- Added a safety net: if the AI's first attempt is off by more than 25% from
  the target runtime, the app automatically asks it to try again once
  (never more than once, to protect the daily free-tier quota) with the
  actual vs. target numbers spelled out.
- Fixed a real bug found while testing a 120-minute film: the AI's response
  got cut off mid-JSON because the token budget was fixed regardless of how
  much content a long film actually needs. The budget now scales with the
  target runtime instead.
Tested end-to-end: a 20-minute series episode went from 5 scenes to 9
scenes summing to 19.5-20 minutes; a 12-minute short film came back as 6
scenes summing to exactly 12 minutes with lean 2-beat acts; a 120-minute
feature came back as 56 scenes summing to 112 minutes (well within the
tolerance, so no retry was even needed) with a richer 5-beat three-act
structure and real subplots. Verified all of this live in the browser too,
not just via direct backend checks.

**Update 2026-08-12 (full screenplay draft, scene by scene):** Once the
scene list is approved, each scene now shows a "Write This Scene" button.
Clicking it expands that ONE scene into real screenplay format — action
lines plus character dialogue — matching the agent spec's explicit
instruction to write "scene-by-scene, not the whole film at once." This is
also a deliberate quota decision: each scene is its own AI call, and on
Gemini's free tier (20 requests/day) a one-click "write everything" button
for a 40+ scene project would burn most of a day's quota instantly, so the
user stays in control of the pace. A new `screenplay_scenes` table stores
one row per scene (episode + scene index for a series, just scene index for
a film); writing a scene shows the AI the full scene outline plus the
immediately preceding scene's already-written content, so character names
and voice stay consistent from scene to scene without needing a separate
"character bible." Each scene has its own Request Changes box (feedback
regenerates just that one scene, non-destructively — old draft kept). A
running "Screenplay progress: X / Y scenes written" line tracks how much of
the project is drafted, and once every scene has a draft it's replaced with
a completion banner (a nod to Instruction 6 — signaling the stage is done
and ready to hand off — though there's no Hero Agent yet to actually hand
off to).
Tested end-to-end on a real 41-scene, 4-episode series: wrote a scene via
the actual "Write This Scene" button in the browser and confirmed it
genuinely continued the previous scene's moment (referencing the isolation
established one scene earlier), requested changes on an earlier scene
("mention his late father's name, Kanhu") and confirmed the new line landed
in exactly the right place, and confirmed clean bilingual output with zero
foreign-script leftovers (scanned programmatically) plus correct rendering
in both languages live in the browser, including the progress counter
updating correctly.
Note: character name cues (e.g. "BIRA") currently stay in Roman script even
when viewing in Odia, since names aren't translated by design — but real
Odia screenplays often write the cue itself in Odia script too. Flagged as
a possible refinement, not changed yet, since it touches how character
identity is tracked across scenes.

**Agent 1 complete when**: you can type a concept, get storylines, approve
as Producer, get a three-act structure, revise and lock it as Story Writer,
then generate scene one-liners and a full screenplay as Screenplay Writer —
all saved and reloadable. No other agents exist yet — that's correct at this stage.

**This milestone is now reached** (2026-08-12) — every piece of that pipeline
exists and works end-to-end, including the runtime-matching and Odia-quality
fixes along the way. Writing every single scene of a full-length project is
still a manual, scene-by-scene process by design (see the note above on
Gemini's free-tier quota), but the capability itself is complete. Agent 2
(Hero Agent) can be started next whenever the user is ready.

---

## AGENT 2 — Hero Agent (simple router version only)
Full spec: `agents/hero-agent.md`

- [ ] Not started
- [ ] Goal for this stage: only build save/handoff logic — take Agent 1's finalized output and mark the project ready for the next stage. No smart coordination yet.

---

## AGENT 3 — Storyboard Agent
Full spec: `agents/storyboard-agent.md`

- [ ] Not started — begin only after Agent 2 (simple router) works

---

## AGENT 4 — Production Management Agent
Full spec: `agents/production-management-agent.md`

- [x] Part 1 (Shoot Schedule — Instructions 1/2/4) DONE, built ahead of
  Agent 2/3 at the user's explicit request — see the full write-up near the
  top of this file. Wired directly to Agent 1's scene list, skipping the
  Hero/Storyboard handoff for now.
- [ ] Part 2 (Budget Allocation — Instruction 3) — not started, next up.
- [ ] Instruction 5 (signal completion to Hero Agent) — deferred with the
  rest of the Hero Agent build.

---

## LATER PHASES (not yet broken into agent form)
- [ ] Casting matching (likely folds into Production Management Agent — decide when you get here)
- [ ] Crew Communication (WhatsApp) — likely its own agent or service
- [ ] Post-Production Tools (promo, censor docs, subtitles) — likely its own agent

---

## Session Log
*(Add a one-line entry here every time you finish a session, so you can see progress over time)*

- Project created — roadmap and tracker set up. Ready to begin Step 1.1.
- Step 1.1 done — installed Node.js, built the React + Node.js skeleton, confirmed it runs locally in the browser. Next: Step 1.2 (concept input page).
- Step 1.2 done — built the concept text box + Generate button, verified the click triggers a display update. Next: Step 1.3 (connect to Claude API).
- Step 1.3 stubbed — wired Generate button to a real backend API call, but backend returns fake storylines (no Anthropic key yet). Next: Step 1.4 (save to database), with real Claude API as a follow-up whenever ready.
- Step 1.3 completed for real — switched AI provider from Anthropic Claude to Google Gemini (free tier, no billing needed), wired up a real API key, tested live in browser with genuine AI-generated storylines. Tech stack updated in CLAUDE.md. Next: Step 1.4 (save to database).
- Step 1.4 done — installed PostgreSQL via Postgres.app, created the `concepts` table, wired the backend to save every generated concept + storylines and reload the latest one on page load. Verified with real page refreshes. Next: Step 1.5 (pitch deck view + export).
- Step 1.5 done — added "Choose this one" per storyline, which generates a full pitch deck (premise, tone/genre, target audience) via Gemini, saves it to a new `pitch_decks` table, displays it cleanly, and exports it as a real downloadable PDF. Verified the PDF opens correctly. Next: Step 1.6 (approval flow as Producer).
- Step 1.5 revised — switched storyline/pitch-deck generation to be Odisha-culture-grounded (Ollywood style, not Hollywood) and fully bilingual (English + Odia), with a language toggle in the UI and Odia-script PDF export (embedded Noto Sans Oriya font). Tested both languages end-to-end, including PDF rendering. Next: Step 1.6 (approval flow as Producer).
- Step 1.5 revised again — added the film/web-series format selector (with episode count + length for series) and rebuilt the PDF export as a proper landscape, multi-slide presentation deck with genre-based color themes and better typography. Tested with both a film and a 15x10min web series, in both languages. Next: Step 1.6 (approval flow as Producer).
- Step 1.5 revised once more — matched the design to a real reference pitch deck the user shared (checked AI image generation, but it needs paid billing, so stayed free); added a punchy display font, per-episode breakdown pages/content for web series, and a closing slide. Fixed a duplicate-episode-numbering bug found during testing. Next: Step 1.6 (approval flow as Producer).
- Fixed a real bug: hit Google's free-tier daily cap (20 requests/day) on `gemini-flash-latest`, which caused the whole page to go blank on any AI error (frontend crashed instead of showing a message). Switched the model to `gemini-flash-lite-latest` (separate quota, worked immediately) and added proper error handling in the frontend so future hiccups show a message instead of crashing. Confirmed fixed live in browser. Session ended for the night here — next up whenever we resume: Step 1.6 (approval flow as Producer).
- Step 1.6 done — added Approve/Request Changes buttons, a feedback box that regenerates the pitch deck (as a new version, old one kept with the feedback recorded), and an "✅ Approved as Producer" badge once approved. Verified end-to-end: feedback genuinely changed the tone, and the approved badge survived a page refresh. Next: Step 1.7 (three-act structure generation).
- Step 1.7 done — added "Generate Three-Act Structure" (only shown once approved), reusing the Step 1.3 API-call pattern to get Setup/Confrontation/Resolution with key beats, bilingual. Verified end-to-end including page refresh and language toggle. Next: Step 1.8 (revision + lock for the three-act structure).
- Step 1.8 done — added Request Changes/Lock Structure buttons for the three-act structure (same revision pattern as Step 1.6), plus a Version History list where every past version stays viewable (never deleted) with a View/Hide toggle per version. Verified end-to-end in the browser: a revision genuinely added a new plot beat, locking survived a page refresh, and an old version's un-revised content correctly reappeared when expanded. Next: Step 1.9 (scene one-liners and full screenplay).
- Step 1.7 enhanced — for web series, the three-act structure now also generates a mini three-act breakdown per individual episode (not just one structure for the whole series), reusing Step 1.8's revision/lock/history system automatically. Tested with a real 3-episode series in both languages, including a revision that correctly targeted one specific episode. Next: Step 1.9 (scene one-liners and full screenplay).
- Step 1.9 started (scene one-liners half done) — once a three-act structure is locked, a new "Generate Scene One-Liners" button breaks it into a full scene-by-scene list (per-episode for a series, one flat list for a film), each scene with a heading and bilingual one-liner, with its own Approve/Request Changes step as Screenplay Writer. Tested end-to-end with a 3-episode series: a revision correctly inserted a new scene in the right place, and everything survived a full page reload in both languages. Next: the full screenplay draft (second half of Step 1.9).
- Added an Odia script cleanup filter — strips stray characters from other scripts that occasionally slipped into AI-generated Odia text. Caught and fixed a near-miss where the first version of the filter would have deleted the Odia sentence-ending punctuation mark too. Verified clean with a fresh regeneration, scanned programmatically. User is now testing scene one-liners themselves before the full screenplay draft is built.
- Fixed a real pacing bug the user caught: scene one-liners were being generated with a fixed low scene count, ignoring the actual runtime — a 20-minute episode only got 5 scenes. Added a Total Runtime field for films (series already had episode length), made every scene carry its own estimated duration with a running total shown against the target, made the three-act structure scale its beat count to the runtime (lean for a short film, richer for a feature), and fixed a token-budget bug that silently truncated a 120-minute film's scene list. Tested a 12-min short, a 20-min series episode, and a 120-min feature — all landed within a few minutes of their target. Next: the full screenplay draft (second half of Step 1.9).
- Created `reference-screenplays/` (with `odia/film`, `odia/series`, `other-languages` subfolders + a README) as a drop-off point for real screenplays the user wants to share, so Claude can later study natural Odia dialogue and story structure and use it to make the app's writing less robotic. Waiting on the user to add files before any analysis can happen.
- Refined the reference-screenplays structure: split `odia/film/` into `hits/` and `average/` (so a hit and an average film can be compared side by side to see what actually makes the difference, not just what Odia dialogue looks like in general) and added a separate `odia/telefilm/` category, since telefilms are a distinct shorter TV-broadcast format with different pacing from a feature film or series episode.
- Step 1.9 finished — built the full screenplay draft feature: once a scene list is approved, each scene gets a "Write This Scene" button that expands it into real action lines + dialogue, one scene at a time (by design, both per the agent spec and to keep control over the daily AI-quota cost). Each scene has its own Request Changes box, and a progress line tracks how much of the project is drafted. Tested live on a real 41-scene series: a written scene correctly continued the previous one, a revision landed in exactly the right spot, and bilingual output stayed clean. This completes Agent 1's full pipeline end-to-end. Next: Agent 2 (the Hero Agent), whenever the user is ready to start it.
- First reference-screenplay analysis pass — user added real Odia web series scripts into `odia/series/`. Read a sample across several, rewrote the screenplay dialogue prompt to match real patterns (broken/imperfect sentences, natural English code-switching, register varying sharply by character, constant family-address terms). Caught and fixed a real regression during testing: the AI briefly started writing the Odia field in Romanized Latin letters (mimicking how some real scripts are written for on-set use), which broke the actual bilingual requirement — fixed by clarifying that code-switching means an English word inside an Odia-script sentence, not the whole sentence in Latin. Verified with a side-by-side regeneration of the same scene. Only `odia/series/` has content so far; film/telefilm/hits-vs-average still empty.
- User shared a big blueprint for a full chat-style UI rewrite + a Bit Sheet stage + a quality-scoring engine + natural-language stage-jumping, all as one request. Split it into four separate pieces instead of building it all at once, and built the Bit Sheet stage first (see Step 1.8b) since it was lowest-risk and fit the existing architecture. The other three are still just proposals, not built:
  - **Chat-style UI rewrite**: biggest/riskiest — would replace most of the working UI from Steps 1.1-1.9. Needs its own scoping conversation, likely its own multi-session project.
  - **Quality scoring engine** (X/10 + suggested fixes per stage): needs a design decision on whether "Auto-Apply Fixes" should ever change content without explicit review (tension with the "never skip an approval step" rule already followed everywhere else), plus a plan for the extra Gemini-call cost against the free-tier quota.
  - **Natural-language stage-jumping**: overlaps with Agent 2 (the Hero Agent), which the roadmap already scoped as "simple router, no smart coordination yet" — would need deciding whether to expand that scope now or later.
  Revisit these whenever the user wants to pick one up.
- User added 5 classic screenwriting craft books (Field, McKee, Snyder, Trottier, Vogler) into `screenwriting-craft-guides/`. Ran 5 parallel research passes reading each book and comparing its framework against our actual pipeline. Five concrete recommendations came out of it (expand Bit Sheet beat types, add a Theme/Controlling Idea field, require a "turn" per scene, add a structured Characters layer, expand screenplay formatting support) — none built yet, waiting on the user to prioritize. The single most valuable gap found: we have no structured Characters concept anywhere — character is just a free-text name string on dialogue lines.
- Started "build all one by one" on the 5 craft-book recommendations: #1 (Bit Sheet structural anchors) done and tested; #2 (Theme/Controlling Idea field) code-complete but not yet tested — paused mid-verification when the user redirected to UI work instead.
- Pivoted to a UI redesign per explicit user request: converted the dashboard-style "show everything stacked" layout into a stage-based one — a left sidebar with an Idea/Synopsis/Bit Sheet/Screenplay navigator (done/current/upcoming states, click to jump), main content showing only the active stage, a minimal single-bubble-plus-input first screen, and Enter-to-submit on the idea box. Kept the "skip ahead" mechanism deliberately simple (navigate freely, get a clear message + one-click link back if a stage's prerequisites aren't ready yet) rather than building auto-chaining or NL routing. Tested end-to-end against a real, live-progressing project. Next: a right-side action panel (some ambiguity in the requested items to resolve), then real multi-project switching.
- User reviewed the stage-nav redesign live and said directly it was "a messed up ui," then shared a real Gemini UI screenshot plus custom icons and a precise respecification: no stage tracker, a genuine scrolling chat log, one persistent contextual input bar, and side-by-side storyline options with a locked/dimmed visual state. Confirmed via two quick questions (Gemini-style only, no stage tracker; icon placement left to Claude's judgment) before rebuilding. Removed the stage-nav entirely and rebuilt: empty-state gradient greeting, persistent bottom input bar that switches between "generate" and "regenerate" modes depending on whether storylines already exist, side-by-side option cards with a "✓ Locked in" badge + dimmed sibling, and a full dark Gemini-inspired color theme applied across every existing card (pitch deck, three-act, bit sheet, scene list, screenplay), not just the sidebar. Also fixed a leftover Vite-starter default (`#root` boxed at 1126px with side borders) that was fighting the new full-bleed layout. Tested the entire flow live in the browser end-to-end — matches the user's spec exactly. Next: the right-side action panel, then real multi-project switching.
- Fixed the "always shows the last project" bug and built real multi-project support: found the actual root cause (every `/latest` backend endpoint grabbed the single newest row anywhere, ignoring which project it belonged to), added scoped endpoints (list all projects, load one project's full chain by id, rename), and wired the sidebar's History section up to them for real Load/New project switching — "New Idea" now also clears its `localStorage` pointer so a refresh actually stays fresh. Brought back a stage-progress list in the sidebar (Idea/Synopsis/Bit Sheet/Screenplay, done/current/upcoming) that smooth-scrolls to a section on click instead of replacing the chat log, learning from what actually went wrong with the earlier reverted stage-nav. Made the persistent input bar truly persistent — it no longer disappears after the synopsis, and now routes typed feedback to whichever stage is currently awaiting a decision. Also fixed the sidebar visibly scrolling away with the main content (an `.app-shell` sizing bug) so it now stays fixed. Caught and fixed a real bug during testing: loading a project from History lost the in-memory "chosen storyline" marker, which briefly made the input bar wrongly offer to regenerate storylines even when a pitch deck already existed. Tested end-to-end live in the browser. Rename is code-complete but needs the user's own manual test (uses a native prompt() dialog the automated browser can't drive). Next: the right-side action panel.
- Big batch: added Export Idea/Import Idea (download/reload a project as a `.json` file on your own computer), History pin and delete (changed the database foreign keys from `ON DELETE SET NULL` to `ON DELETE CASCADE` so deleting a project cleanly removes its whole chain), and "skip ahead" — a "Start from: Idea/Synopsis/Bit Sheet/Scene One-Liners" tab row that lets you paste your own already-written material and have ONE AI call both faithfully parse it into the real schema AND invent plausible, consistent earlier stages behind it (film format only for now). Also moved the film/web-series question to the very first screen (before typing an idea at all) instead of after choosing a storyline, and threaded that format into storyline generation itself — choosing a storyline now goes straight to building the pitch deck since format is already known. Tested export→import round-trip, cascade-delete, pin-to-top, the full format-first flow, and a real skip-to-bitsheet paste end-to-end live in the browser; skip-to-synopsis/skip-to-scenelist verified by code review only to conserve the day's Gemini quota (same proven one-call pattern). Noted one rare AI glitch (a stray Hebrew character in one English title) — not chasing it as a general fix since it's a one-off. Rename/delete confirmation dialogs are native browser prompts the automated test browser can't drive — worth the user's own click-through. Next: the right-side action panel; trying rename/delete/skip-ahead (all 3 target stages) yourself.
- User asked to "make it online" so work could continue from a chat session instead of a local laptop. Rather than deploying from scratch, checked each piece first and found it was mostly already live from earlier work: Supabase database + `crew-photos` storage bucket both set up, a Render backend already auto-deploying from `main` (https://filmybase.onrender.com, confirmed healthy), and a Vercel frontend already auto-deploying from `main` (https://filmybase.vercel.app) with `VITE_BACKEND_URL` already pointed at the Render backend. Confirmed for real by opening the live site and generating actual storylines from a typed concept. Nothing needed building — just verified end-to-end and documented it here since it hadn't been logged before. Next: keep building features from this chat; any push to `main` auto-deploys within a minute or two, no manual redeploy step needed.
- Fixed two real navigation bugs the user caught: (1) clicking "New Idea" after using a "Start from: Synopsis/Bit Sheet/Scene One-Liners" skip-ahead tab left you stuck on that tab instead of returning to the plain Idea/format-picker screen — root cause was `handleNewIdeaClick` resetting every piece of project state except which "start from" tab was selected; now resets that too. (2) the "Filmmaking App" logo/title top-left in the sidebar did nothing when clicked; it's now a real button that acts like "New Idea" and also switches back to the Story & Screenplay agent, so it's a genuine "go home" click from anywhere in the app (including from the Production Management agent). Set up a local Postgres + seeded test admin login to actually run the app and click through both fixes live in a browser rather than just reading the code, screenshotted before/after for each. Next: whatever the user wants to tackle next.
- User asked to deeply investigate Android app bugs; investigation was cut short when they immediately spotted the actual top complaint themselves: the "Changes" chat box (where you type your idea or any change/feedback) was showing up as a permanent right-hand column on desktop — eating a quarter of the screen "disturbing" every other view — instead of the small pop-out box it was designed to be. Root cause: a `@media (min-width: 901px)` CSS rule turned it from a floating popup into a fixed sidebar column specifically on wider screens, which is also likely why it looked wrong on some Android devices/orientations. Removed that rule entirely — the "Changes" box is now ALWAYS a small floating popup (opened by clicking the pen icon bottom-right, closed by default), on every screen size, mobile and desktop alike. Also shrank it from a full-height edge panel to a genuinely small anchored box (360×480px max, smaller on phones) so it reads as "pop out a box to type," not "sidebar." Verified live in a browser at both a desktop width and a phone width. Not yet done: the deeper Android-specific investigation (back button behavior, on-screen keyboard covering inputs, safe-area insets) — paused when the user redirected here; worth returning to since those are real, found-but-unconfirmed issues. Next: ask the user if there are other specific Android bugs beyond this one, or go build out that deeper Android review.
- Follow-up: user wanted the pen icon (that opens the Changes box) to be a genuinely floating, draggable button — like the existing auto-pipeline widget already is — instead of nailed to a fixed screen corner. Reused that exact same drag mechanic (pointer-down + move + up, position remembered per-viewer in localStorage, a 5px move threshold so a plain tap still opens the box instead of being swallowed as a drag) as a small shared helper, and wired it into the pen button. Removed the old hover-to-open behavior along the way since it doesn't make sense once the button can be dragged (hovering to grab it would have popped the box open first). The box itself still opens in its usual spot near the bottom-right regardless of where the pen gets dragged to — didn't try to make the box itself follow the button, since that's a bigger change than what was asked. Verified live: dragged the pen across the screen, confirmed a click afterward still opens the box (not swallowed as a drag), and confirmed the dragged position survives a page reload. Next: still open — the deeper Android bug review (back button, keyboard-covering-input) mentioned above, whenever the user wants to pick it up.
- Merged the branch into `main` at the user's request so all of the above actually went live on filmybase.vercel.app (it had only existed on the working branch until now). Going forward, the plan is to keep doing this automatically once a fix is built and verified, instead of asking each time.
- User described a much bigger reshape of the Idea step (a chain: click Idea → popup with a "Generate Idea" button → 3 ideas appear → a "Generate more" that keeps old batches visible instead of replacing them → picking two ideas from different batches to ask for a merge). Built the first, smallest slice of it rather than all of it at once: clicking "Idea" in the sidebar now pops the Changes box open directly (it used to just scroll to that section), and its send button is now labeled "Generate Idea" specifically on that stage instead of a plain arrow. Also changed the storyline count from "2-3" (the AI's own judgment call) to exactly 3, matching what was asked. The batching/"generate more without losing old ones"/merge-picker part is real, substantial, follow-up work — not built yet, flagged as next.
- Fixed a real bug the user caught while testing the above: a brand-new project has no id yet until its first successful generation, so its Changes-box history was filed under a shared placeholder key ("new:idea") — meaning starting a fresh idea after abandoning a previous *unsaved* one would resurface that old attempt's messages, since both shared the same placeholder. Once a project is actually saved (real id), its history was already correctly private to that project — that part was never broken. Fixed by having "New Idea" genuinely delete the placeholder bucket (not just hide it) — verified by inspecting localStorage directly before/after, not just visually. Next: the batching/merge feature described above, whenever the user wants to scope it further.
- Turned out the previous entry's "click Idea" fix was for the wrong UI element — user actually meant the **"Start from: Idea / Synopsis / Bit Sheet / Scene One-Liners"** tab row in the main screen (a separate, older feature — "skip ahead" — that lets you paste already-written material at a later stage), not the sidebar nav item. Confirmed exactly what was wanted via 3 quick questions rather than guessing on a request that mixed two different UI pieces together. Fix: the "Idea" tab in that row now shows its own paste box too, just like Synopsis/Bit Sheet/Scene One-Liners already did — labeled "Paste your idea here" with a "Generate Idea" button — instead of uniquely bouncing back to the empty/format-picker screen with no visible box at all. It reuses the exact same generate-storylines flow as the pen tool's Idea box (same `concept` state, same `handleGenerateClick`), not the "invent earlier stages" skip-ahead logic the other 3 tabs use, since Idea has no earlier stage to invent. Confirmed the pen tool's per-stage history should stay separate (not combined into one timeline) — no change needed there. Verified all 4 tabs live in the browser. Next: voice-to-text input the user just asked for, then the Idea batching/merge feature.
- Added voice-to-text (dictation) so typing isn't the only way to fill in any writable box. Uses the browser's own free built-in speech recognition (Chrome and the Android app's WebView both have it — no signup, no API key, no per-word cost) rather than a paid service. Real limitation explained to the user first: one recognition session has to be told a single language up front — it can't automatically tell English apart from Hindi mid-recording the way they first pictured, so a small EN/HI/OR picker sits next to every mic and remembers the last choice. First built it for the Idea box (pen tool + the new paste box) alone, but the user then asked for a mic "wherever there is an option to write, everywhere in the whole app" — so went back through the entire file and added one to every genuine free-text field: every "Request Changes" box (pitch deck, characters, three-act, bit sheet, scene list, breakdown, schedule), both Import/Re-import Screenplay paste boxes, the production chat ("master agent"), the auto-pipeline widget's concept box, crew/cast name fields, the Script Breakdown category editor's every text field (locations, costumes, cast, notes, in all three languages), character/location availability dates, per-scene costume/properties/AD-remark edit fields, and the shoot-day completion report boxes. Deliberately left out: login credentials, new-user username, and the Digital Clapboard's tiny numeric slate fields (scene/shot/take/date) — voice dictation doesn't make sense for typed passwords or single-digit slate numbers. Built two reusable drop-in pieces (`MicInput`, `MicTextarea`) plus a shared "dictation language" setting (React Context, so it didn't need threading through every component by hand) to make ~40 conversions mechanical and low-risk rather than one-off hacks — confirmed with a clean build and by checking it live on two different pages. Could not visually check every single one of the ~40 fields (many only appear once a project has real AI-generated content, which needs a live Gemini key this sandbox doesn't have) — same tested component everywhere, but worth the user flagging anything that looks off.
- User couldn't tell whether "Analyze Script"/"Re-analyze" (Script Breakdown, in Production Management) was actually doing anything — it was just a button that went disabled with a static "Analyzing..." label, no other sign of life, for however long one Gemini call takes. Added a real progress indicator: a continuously-sliding bar (honest about not knowing the real percentage — there's no staged backend progress to poll here, unlike the auto-pipeline widget's multi-stage run, so it never fakes a number) plus a live "· Ns" elapsed-time count, shown under both the first-time "Analyze Script" button and each category's "Re-analyze" button. Verified live by seeding a local test project directly in the database (skipping the Gemini call, which needs a real key this sandbox doesn't have) and artificially delaying the network response to actually watch the bar move and the counter tick up. While testing this for real, the user hit a real `429 Resource exhausted` error from Gemini — that's Google's free-tier quota/rate limit, not a bug in the app; explained that distinction so they know whether it's worth waiting out or upgrading the Gemini plan. Next: whatever the user wants to pick up after re-testing.
- User then found the same "is this actually happening" problem in more spots (Re-upload Updated Screenplay, in that same test), and asked for a real moving percentage (like "10%, 15%, 20%...") everywhere something is analyzing or uploading — not just a sliding bar with no number. Upgraded `AnalyzingProgressBar` to show an estimated percentage: it climbs fast at first then eases off, capped at 95% until the real response actually lands (never claims 100% early, never goes backwards). Being upfront about the honesty of this: none of these are staged backend jobs like the auto-pipeline widget's real multi-stage run — they're each one blocking AI call with nothing real to poll — so the % is a reasonable estimate based on how long that particular call usually takes, not a true measurement. Then went through the whole app adding it everywhere a long AI call was previously just a disabled button with a static label and nothing else: AD Sheet generation, costume recommendations, missing-character scan, cast classification, shoot-day report reading, Import/Re-import Screenplay (both already had the bar, now with %), and on the Story & Screenplay side — pitch deck, characters, three-act, bit sheet, scene list, and writing an individual screenplay scene — plus the shared "Changes" pop-up's busy state, which covers every "request changes"/regenerate submission in one place. Left the two chat panels' bouncing "thinking" dots alone (pen tool's general typing indicator, production chat) since a bouncing-dots "typing" animation is already the right, recognizable pattern for a conversational chat bubble, not a form submission. Verified the percentage genuinely climbs over real elapsed time (5% at 1s, 18% at 4s) and disappears cleanly once the call resolves, using the same delayed-network-response trick as before. Next: whatever the user wants to pick up after re-testing the upload.
- Fixed the Story & Screenplay Agent based on two independent critiques of a real generated script ("The Illusion of Amulya") — Claude's own read plus a separate rating from Google Gemini. Four real issues confirmed and fixed in the agent's prompt/schema (a 5th claim from Gemini, about sluglines splitting across lines, could not be confirmed against either the actual PDF or the export code, so it was left alone rather than "fixed" blind): (1) the literal word "CHARACTER" sometimes printing instead of a real speaker name — root cause was the AI schema not requiring a "character" field on dialogue elements, so it could omit it; now required in the schema plus an explicit instruction that every dialogue line must name a real speaker. (2) unpaid-off props/reveals — added an instruction telling the AI to actually use the full scene outline it's already given to check that objects/secrets set up in one scene are honored later, instead of writing each scene in isolation. (3) passive-protagonist, "telling not showing" action lines — added a show-don't-tell rule (dramatize emotion through action, never name it outright) and a rule that the protagonist keeps some real agency/reaction even in a scene where they're being outmaneuvered. (4) Odia grammar/spelling mistakes — added an explicit self-proofread pass to the Odia dialogue instructions covering the specific mistakes AI Odia text tends to make (wrong vowel signs, dropped nasal marks, wrong conjunct letters, wrong case-particle attachment, verb-ending/respect-level mismatches). Verified the server still starts cleanly (syntax check) after the prompt/schema edits; a real before/after quality comparison needs the user's own Gemini key to regenerate a scene, since this sandbox has none. Next: the user's real reference screenplays (mentioned but not found in this session's files) still need to be located/re-uploaded and folded into the Odia-quality instructions as concrete grounding, once available.
- Chased a real "Script Breakdown stuck at 95%" bug across several wrong turns before finding the actual cause, on a real 13-episode/166KB script the user was testing ("Hajiro"). Along the way: switched the app over to real Vertex AI billing (a service-account JSON env var the code already supported but wasn't wired up on Render yet — added a startup log line so it's now visible in Render's logs which backend is active, and documented the env vars in `.env.example`), added a timeout to the underlying Gemini call (there wasn't one at all — a genuine stall had nothing to catch it and would hang forever), and added a tiny isolated `/api/test-gemini` endpoint that confirmed the Vertex connection itself was healthy and fast (2.3s for a small request). None of those were the real bug. The user pasted the actual Render logs, which showed it: the category-by-category "double-check" re-analysis pass sends the WHOLE script in one call and asks for one complete JSON list back with no length cap — on a script this size the response (170,000+ characters) reliably got cut off mid-string before it could finish, on every single retry attempt, so that category never actually completed no matter how long it waited. Fixed for real this time: that re-check now runs per-episode instead of over the whole script at once (reusing the same episode-splitting and "only ever add what's missing, never touch what's already found" pattern already used elsewhere for exactly this kind of large-script problem), so each response stays small and reliable. Verified the merge/dedupe logic with a standalone test (case-insensitive matching per category, malformed entries skipped safely, nothing already-found ever lost) since this sandbox has no real Gemini/Vertex key to test the actual API calls end-to-end. Next: have the user retry "Analyze Script" on Hajiro again — this is the first fix aimed at the actual proven cause rather than a plausible-sounding guess, so it's the one to watch closely.
- Added Lead / Sidekick / Extra (Junior) / Non-Speaking cast tiers to the Artist List, per the user's explicit spec (confirmed understanding before building, per this file's own working rule). Extends the existing "Classify Cast Categories" AI pass with a new judgment — which speaking characters are leads, sidekicks, or minor/extra roles — decided in one whole-script call rather than chunked per-episode, since "how important is this character to the story" is a holistic judgment (unlike the existing speaking/non-speaking evidence, which is naturally gathered per-episode and ORed together). Changed how casting works per tier: Lead/Sidekick still get cast one actor at a time as before; Extras/Juniors no longer need individual casting at all — adding ONE "Junior Artist Coordinator" now marks all of them cast at once; Non-speaking characters are left out of casting entirely (shown in the breakdown, but no cast action, no pending/finalized chip). Set up a local Postgres + a seeded test project spanning all four tiers and drove it through a real browser end-to-end (not just reading the code) — this caught a real bug the code-read alone would have missed: the "Production Status Overview" panel's cast count/pending-list was computed separately from the artist-list section and knew nothing about the new tiers, so it kept nagging about characters who were actually already covered (non-speaking ones that don't need casting, extras already covered by the coordinator). Fixed that same computation to match. Verified live: Lead/Sidekick keep individual casting, Extras show no individual form and all flip to "cast" together the moment the coordinator is added, non-speaking characters show no casting UI at all, and the overview panel's numbers/list now agree with the artist list itself. Next: whatever the user wants to pick up after trying this themselves.
- User caught a real bug in the Lead/Sidekick/Extra feature on the actual live Hajiro project (87 cast members) — Lead and Sidekick groups weren't showing up at all, everyone speaking landed in Extras. Root cause: the AI tier-classification step asked the model to repeat each character's name back alongside its tier, then matched by that repeated text — at a real 87-person cast list, small text differences (a respelling, a dropped title, different capitalization) broke the exact match for nearly everyone, silently defaulting them all to "extra" with no error anywhere to notice. Fixed by making it positional instead: the AI just returns a plain list of tiers in the same order it was given the character list, matched back by position rather than by re-typed text — removes that whole failure mode. Verified the fix's logic directly (not against the real API, which needs a real key this sandbox doesn't have) with a standalone test covering the normal case and the "response came back a different length" safety fallback. Also built the episode-number request from a couple of messages back: every category (characters, locations, props, costumes, art) now gets tagged with which episode(s) each item appears in — kept to episode-level only (not exact scene numbers) since the user explicitly chose the cheaper/faster option once I explained scene-level would need its own slow extra pass. Runs automatically at the end of a fresh "Analyze Script," plus a new standalone "Tag Episode Numbers" button (only shown for series projects) so an already-existing breakdown like Hajiro's (built before this shipped) can pick it up without starting over. Set up a local test project again and confirmed live: episode badges show correctly next to every item in every category, the button only appears for series (not single-episode films), and everything coexists cleanly with the Lead/Sidekick grouping. Next: have the user try "Tag Episode Numbers" on the real Hajiro project and confirm Lead/Sidekick actually populate correctly this time.
- Hajiro STILL showed only Extra/Non-Speaking after the last fix — turned out the fix itself was right, but it only corrected *how* tiers get matched; it never actually re-ran on Hajiro's already-existing breakdown, since Lead/Sidekick classification was a separate manual "Classify Cast Categories" click nobody remembered to make again. The user was explicit this time: it should just be done when they open the app, nothing to click. Closed this for good two ways — (1) a brand-new "Analyze Script" run now does cast-tier classification automatically as part of the same pipeline that already auto-tags episode numbers, no separate step at all; (2) for a project analyzed before this shipped (Hajiro exactly), the page that loads a project's data now silently checks in the background whether anything's missing a tier or episode numbers, and if so kicks off fixing it right then — never slowing down the page itself, and safe to hit repeatedly (won't fire the same expensive check twice at once, and a failed attempt doesn't get stuck, it'll just try again next time the project is opened). Verified locally that a page load stays instant either way and a failed background attempt (tested against a fake key, since this sandbox has no real one) logs cleanly without breaking anything. Next: this is the one that should actually make Hajiro self-heal just by opening it again — worth the user confirming Lead/Sidekick/episode numbers show up correctly without touching any button this time.
- User asked to just have Hajiro re-analyzed directly and confirmed done — had to be upfront that this session has no network access to the live Render backend or Supabase database, and can't run anything against production directly or verify its outcome. The real gap behind the repeated "still not fixed" reports: the previous auto-backfill was completely silent — it may well have been working correctly the whole time, just taking a few minutes, with absolutely no way for the user (or anyone reading logs remotely) to tell "still working" apart from "actually broken." Fixed that blind spot instead of guessing at the fix again: the backend now reports a real status (working / retrying after a failure / nothing to do) alongside the breakdown, and the app shows a plain banner on the Script Breakdown section whenever it's actively updating in the background, quietly refreshing itself once done — no more silent guessing on either side. Verified the banner renders correctly against a seeded stale breakdown. Next: genuinely can't confirm this from here — needs the user to open Hajiro and see for themselves whether the banner appears and whether it clears with the fix actually visible.
- Built (and then fully reverted, at the user's explicit request) a "Project Instructions" admin panel for layering extra creative direction onto one project's existing agents. Reverted cleanly — the commits, the database column, the UI, all gone, and confirmed the other session's unrelated quota-error-message fix (which had landed on `main` in between) stayed intact. Reason: the user's real need turned out to be much bigger than "extra instructions on the same agents" — they're starting a new project ("Akhara") that's an AI-generated movie, not a physically shot one, so the entire post-screenplay pipeline needs to be genuinely different (no crew, no casting, no shoot schedule — an AI-generation pipeline instead). Proposed idea: a new project FORMAT option ("AI Movie") alongside Film/Web Series/Vertical Drama, with its own dedicated agent replacing Production Management for that format, sharing the same underlying story/screenplay knowledge base but working completely differently afterward. Not built yet — deliberately paused to scope out what that new stage should actually DO before writing any code, rather than guessing at something this different. Next: figure out with the user exactly what the AI Movie production stage needs to produce (shot-by-shot AI generation prompts? character/location visual reference sheets for consistency? a generation-progress tracker? some combination?) before building anything.
- Built the first, deliberately small step toward the AI Movie idea above, per the user's own explicit instruction: a new "Is this a shooting production, or an AI movie?" choice at the very top of the first screen for a fresh Story & Screenplay project. Choosing "Shooting Production" leaves everything exactly as it already was (format picker, greeting, skip-ahead tabs — untouched). Choosing "AI Movie" swaps in a placeholder panel instead, with no functionality yet — the real AI Movie pipeline is intentionally not built here, since the user stopped a broader build attempt earlier and asked for this toggle first as its own separate step. Verified live in a browser: default view unchanged, switching to "AI Movie" shows the placeholder, switching back fully restores the original screen, and the existing "Bit Sheet" skip-ahead tab (nested in the same area) still works exactly as before with no regression. Next: wait for the user's instructions on what the "AI Movie" panel should actually contain.
- Corrected the placement of the above per the user's follow-up: they wanted the Movie/AI Movie choice right after login — a single global gate for the whole app, not a per-project setting buried inside the Story & Screenplay start screen. Removed the previous toggle entirely (restored the empty-state screen to exactly how it was before) and instead added a real login gate: right after entering username/password, a "Movie or AI Movie?" screen appears. Choosing "Movie" loads the entire existing platform exactly as it's always worked — no changes anywhere. Choosing "AI Movie" opens a genuinely blank panel — no text, no placeholder, nothing — since the user is deliberately designing that pipeline agent-by-agent themselves, one instruction at a time, and explicitly said not to build or guess at anything further until told to. The choice isn't remembered across a page reload on purpose, so reloading the page is always an easy way back to the picker without needing any extra button. Verified live in a browser: the picker appears right after login, "Movie" loads the full untouched app, "AI Movie" shows a completely blank screen, and reloading returns to the picker. Next: wait for the user's next instruction — they said they'll direct exactly what the AI Movie panel and its agents should do, one step at a time.
- Filled in the AI Movie panel's first real pieces, exactly as the user specified after confirming understanding first (per this file's own rule) before building: (1) the same left sidebar as the rest of the app now shows on the AI Movie screen too (logo, user row with working Log out, Manage Users, New Idea, Import/Save Project, language dropdown, an Agents section listing All Projects / Story & Screenplay / Production, and a History section) — but every item in it is currently a plain, non-functional placeholder (no click behavior at all except Log out and the language dropdown, which are basic account/display settings rather than a pipeline "job"); the user will assign real behavior to each one step by step later. Also renamed "Production Management" to just "Production" for this AI Movie context specifically (the shooting-production side keeps its original "Production Management" name, unchanged) — that's where image/audio/video generation will live once built. (2) The main panel now has a paste box + "Analyze" button. Clicking it sends whatever was pasted to a new, single-purpose Gemini call (new backend endpoint `/api/ai-movie/analyze-stage`) that identifies which stage of development the text represents — Concept, Story, Synopsis, Bit Sheet, Screenplay, or "something else" if it doesn't clearly match — and reflects that back in plain words (e.g. "This story is in the Synopsis stage"), then shows a "Proceed" button. Deliberately stopped exactly there per the user's explicit instruction — the Proceed button doesn't do anything yet, and nothing is wired into the sidebar. Verified live in a browser: the sidebar renders correctly (caught and fixed a real layout bug along the way — the "Manage Users" button was missing its wrapper div and inherited `flex: 1` from the sidebar's flex-column layout, stretching it into a giant empty box), all the placeholder sidebar buttons can be clicked with no errors and no effect, the Analyze button correctly reaches the new backend endpoint and Gemini (confirmed via a clean, expected "invalid API key" error, since this sandbox only has a dummy key), and switching to "Movie" mode still shows the original app completely unaffected (still says "Production Management" there, untouched). Not verified: the actual classification accuracy against a real Gemini key, since this sandbox has none — worth the user's own first real test of pasting a synopsis/bit sheet/screenplay and confirming it names the right stage. Next: wait for the user's next instruction on what each sidebar item should actually do, and/or what "Proceed" should lead to.
- Gave the "Proceed" button its first real job, per the user's own spec (confirmed understanding first, built only after they said "commit changes"): it now works BACKWARD from whichever stage Analyze detected and invents whatever earlier layers are missing — Story → Synopsis → Plot (the beat/bit sheet) → Character Arc — while treating whatever was actually pasted as final and authoritative for its own stage: never rewritten, never critiqued, never "judged." Paste a Synopsis and only the Story layer gets invented; paste a Bit Sheet and Story + Synopsis + Character Arc all get invented behind it; paste a Screenplay and everything behind it gets invented. Paste something already at the earliest stage (Concept or Story) and it says there's nothing earlier to fill in; if Analyze couldn't confidently name a stage at all, it says so instead of guessing. Built as a brand-new, separate Gemini system prompt (not reused from the shooting-production agents) carrying the one instruction the user was explicit about: since this is never physically shot, never reason about budget, cast/crew/location availability, or any real-world production constraint — anything imaginable can be included. New backend endpoint: `/api/ai-movie/backfill`. Verified thoroughly: hit the new endpoint directly for every stage and confirmed the earliest-stage and unclear-stage cases correctly skip the Gemini call entirely (instant empty result, no wasted quota), while synopsis/bitsheet/screenplay correctly build their schema and reach Gemini (confirmed via the same clean "invalid API key" error as everywhere else in this sandbox); also mocked a realistic Gemini response end-to-end to confirm the Story/Synopsis/Plot/Character Arc results actually render correctly and readably in the browser. Not verified: real classification+generation quality, since this sandbox has no real Gemini key — worth the user's own first live test. Next: wait for the user's next instruction.
- Big batch, all confirmed with the user before building: (1) added a **silent** asset-extraction agent — never a visible chat bubble, never shown anywhere in the UI right now — that runs automatically right after Proceed backfills, reads everything gathered so far (the pasted material plus any invented Story/Synopsis/Plot/Character Arc), and extracts structured Characters/Properties/Environments lists, each with a vivid visual description. This is preparation for a later Production stage that will turn these into actual image-generation prompts — that later step is NOT built yet, on purpose. (2) Gave the AI Movie pipeline its own real database table (`ai_movie_projects` — pasted text, detected stage, the backfill layers, and the silent agent's asset lists, all in one flat row per project, deliberately simpler than the shooting pipeline's chain of approval-gated tables since this pipeline works completely differently). Every Analyze creates or updates a project row; every Proceed saves the backfill and asset results into it. (3) Wired up the four AI Movie sidebar buttons that were placeholders until now: "All Projects" lists AI Movie projects only (kept completely separate from the regular Movie project list, confirmed with the user first) and loads one back in on click; "New Idea" clears the editor back to a blank paste box; "Import Project" uploads a previously-exported AI Movie project file; "Save Project" downloads the current one. Verified thoroughly against a real local database (not just mocked): imported a project via the API directly, confirmed it round-trips correctly through the detail endpoint, then drove the actual browser through the full loop — All Projects shows the real saved project, clicking it restores the pasted text/detected stage/Story layer exactly, Save Project downloads a file with the correct title and asset data, New Idea clears everything and disables Save Project again — zero JS errors throughout. Also confirmed via request interception that the project id Analyze creates correctly carries through to the following Proceed call, so both stay writing to the same database row. Not yet built: no rename/delete/pin for AI Movie projects (only what was asked), and the reserved `uploads/ai-movie/` image-storage subfolder mentioned in planning was deliberately skipped for now since nothing generates or uploads any image yet — worth revisiting once the Production stage actually needs it. **Needs a manual step from the user**: this sandbox has no network access to the live Supabase database, so the new table has to be created there by hand — the exact `CREATE TABLE ai_movie_projects (...)` statement is in `app/backend/schema.sql` (search for that name) and in this session's chat reply. Next: wait for the user's next instruction — likely what the Production stage should actually do with these saved asset lists.
- Added a "Reference Material" section to the AI Movie editor, right beside the Paste Anything + Analyze box, per the user's own spec (confirmed before building, built once they said "do it"). Lets the user hand the agents real source material directly — a book their story draws from, or their own character/property/art details — instead of leaving everything to invention: pick a category (Book/Characters/Properties/Art/Other), then either paste text or upload a PDF/Word/text file (reused the app's existing PDF/Word text-extraction, already built for screenplay import, rather than adding a new one). Attaching something before any story text has even been pasted quietly creates the project row on the spot, same lazy-create the Analyze button already does. Everything attached to a project now gets read into BOTH the backward-fill agent and the silent asset-extraction agent as grounding material they're told to treat as authoritative — never overridden, never invented around. New table `ai_movie_reference_files` (one project can have several), plus endpoints to add by paste, add by file upload, list, and delete. Verified thoroughly against a real local database: added pasted text and a real hand-built PDF (confirmed actual text was correctly extracted out of the PDF, not just the filename), listed both, deleted one and confirmed only it disappeared, reloaded a saved project from All Projects and confirmed its reference files came back correctly, and confirmed backfill still reaches Gemini cleanly with reference material attached (this sandbox's dummy key can't confirm the agents actually USE it well, only that the wiring doesn't break). Caught and fixed a real bug while testing: pasted reference text with no filename was showing up labeled "Other" in the list — that word was the Other-category name, being wrongly reused as a fallback for "no label given"; fixed by labeling pasted text with a preview of its own content instead, like the project list already does elsewhere. Next: wait for the user's next instruction.
- Two follow-up requests on Reference Material, landed together: (1) broadened uploads to accept several files at once (including a .zip of several documents — each readable file inside becomes its own separate reference entry, so it can be labeled/removed individually) and added `.md`/`.markdown` support (also benefits the existing screenplay-import upload, which reused the same text-extraction code). Added the `adm-zip` package for reading zip archives — nothing else in this app needed one yet. (2) The user then said, mid-build, that they didn't want to hand-pick a category (Book/Characters/Properties/Art/Other) at all — by default it should just analyze and sort itself. Removed the category dropdown entirely and added a small silent classifier (same "never shown as a chat bubble" pattern as the asset-extraction agent) that reads whatever's pasted or uploaded and picks its own category; falls back to "other" on any classification hiccup rather than blocking the save, since getting the material saved matters more than labeling it perfectly. Verified thoroughly: uploaded a .md file, a plain .txt file, and a .zip containing two more documents in a single multi-file selection through the real browser — all four correctly split into separate, correctly-labeled entries with their actual extracted text (confirmed in the database, not just the API response), no category picker anywhere in the UI anymore, zero JS errors. Auto-categorization itself only confirmed to fail gracefully (falls back to "other" cleanly) against this sandbox's dummy Gemini key — its actual judgment quality needs the user's own live test. Next: wait for the user's next instruction.
- Fixed two real bugs the user hit on the actual live site (Vercel + Supabase, from real screenshots). First: "relation ai_movie_projects does not exist" — not a code bug, this sandbox genuinely can't reach the live Supabase database, so the `CREATE TABLE` statements from the last two steps had to be run there by hand; walked the user through doing that in the Supabase SQL Editor, confirmed done from a screenshot showing "Success. No rows returned." Second, a real one this time: after uploading a zip with several files inside (a real "AKHADA_Handoff_Package.zip" test), every single entry in the Reference Material list showed up looking identical — "[category] AKHADA_Handoff_Package.zip → 0…" — impossible to tell any of them apart. Root cause, found by checking the actual CSS: that list was reusing `.sidebar-history-item`, a style built for the 260px-wide sidebar with hard single-line ellipsis truncation — fine there, but in the much wider Reference Material box (and especially on a phone screen, which is what the user's screenshots were) it cut every long label down to almost nothing before the actually-different part ever showed. Fixed two ways: (1) gave the Reference Material list its own CSS (`.ai-movie-reference-row`/`.ai-movie-reference-label`) that wraps instead of truncating, so the full name is always readable; (2) stopped repeating the zip's own filename on every row it contained — each entry now just shows the file's own name from inside the zip, which is what actually varies row to row and is what made the list look like 13 copies of the same thing. Verified live: rebuilt a similar multi-file zip, uploaded it at the same phone width as the user's screenshot, confirmed every entry now shows its own distinct, fully-readable filename. Next: wait for the user's next instruction.
- Closed the gap flagged a few steps back: you can now generate a story straight from Reference Material alone, without pasting anything into the main box first. A new "Generate Story from Reference Material" button appears in the Reference Material section whenever something's attached AND the main paste box is still empty — click it and one agent reads everything attached and originates the full chain in one pass (Story, Synopsis, Plot, Character Arc), faithful to that material, with the same "never think about budget or physical feasibility" instruction every AI Movie agent gets. The generated Story then fills the main box and everything renders exactly like a normal Analyze+Proceed run would, so the rest of the pipeline (and the silent asset-extraction step) doesn't need to know the difference. New backend endpoint `/api/ai-movie/generate-from-reference`. Verified thoroughly: confirmed the button only shows up under the right conditions (hidden with nothing attached, hidden again once the box has text), confirmed the endpoint's two clean-failure cases (no project, no reference material) return sensible errors instead of crashing, confirmed it correctly reaches Gemini with reference material attached (this sandbox's dummy key gives the same clean, expected error as every other agent here), and mocked a realistic response to confirm the full Story/Synopsis/Plot/Character Arc chain renders correctly and the button disappears once it's done its job. Next: wait for the user's next instruction.
- Real discoverability bug, found from the user's own screenshots of their live "Akhada" project: the "Generate Story from Reference Material" button existed and worked, but sat at the very BOTTOM of the Reference Material section, below the whole list of attached files — and their real project had a dozen-plus files attached (a full handoff package zip), pushing the button way off-screen. Instead of scrolling down to find it, the user kept typing an instruction straight into the main "Paste Anything" box ("Start analyzing this" / "Analyse the reference material and update") — which isn't what that box is for (it only classifies whatever's typed as story content), and typing into it also hides the Generate button entirely (it's designed to disappear once you've pasted something, so it never overwrites a real draft), making the real button even harder to find next time. Fixed by moving the button to sit right after the Add/Upload controls, above the attached-files list, and giving it a filled, more prominent look (matching the "New Idea" button's style) instead of blending in with the same outline style as everything else — so it's the first thing visible once you scroll to the section, no matter how many files are attached. Verified against a rebuilt 12-file version of the same scenario (matching what the user's real screenshots showed) at the same phone width: the button now appears immediately above the list, confirmed it's positioned above the first attached item, and confirmed clicking it still works correctly from its new spot. Next: wait for the user's next instruction.
- Once the button was actually visible, the user clicked it on the real Akhada project and got a bare "Something went wrong" — no specific error, which only happens when the request fails at the network level rather than getting a real response back from the server. Likely cause: `getAiMovieReferenceMaterialText` had no size limit at all — it dumped every attached document's FULL content into one blob for the generation prompt, and Akhada's real handoff package has a dozen-plus real documents (full story drafts, scene breakdowns, a whole screenplay act). That's almost certainly too much combined text for the two sequential Gemini calls this feature makes to finish before some layer in between (this sandbox can't confirm exactly which — Render, the browser's own fetch, or Gemini itself taking too long) gives up and kills the connection, which shows up to the user as a network failure with no useful message at all. Fixed with two caps: no single document counts for more than 20,000 characters toward the prompt, and the combined total across all attached documents is capped at 60,000 characters — built by walking newest-first and stopping once the budget's used, so whatever's included stays whole documents (never a mid-sentence cut) and a note is added if anything had to be left out. Verified locally: uploaded 10 large synthetic documents (~300KB combined, matching the real scale of the Akhada project) to one project and confirmed the endpoint still responds immediately and correctly rather than hanging or crashing. Can't confirm from this sandbox whether this was the exact cause of the "something went wrong" the user saw (no real Gemini key here to reproduce actual timing), but it's the most likely explanation and a straightforward, safe fix regardless of the precise cause. Next: have the user try Generate Story from Reference Material on Akhada again — if it still fails, worth getting the real error text (not just "something went wrong") to narrow down further.
- Biggest AI Movie change yet, fully confirmed before building (a 3-question spec check first, since this touches the whole result flow): the pipeline now works stage-by-stage with real review gates, matching exactly how the shooting pipeline's Idea → Synopsis → Characters → Bit Sheet chain already works — instead of Proceed dumping Story/Synopsis/Plot/Character Arc all at once with nothing to approve. Story is still handled the same way as before (from the pasted text, or backfilled behind it, or from Reference Material) and is auto-approved the moment it exists — it's already "final and authoritative," nothing to second-guess. From there, Synopsis, then Plot, then Character Arc, then a brand-new Screenplay stage (not built before at all) each generate ONE AT A TIME — only once the stage before it is approved — using just the already-locked stages as fixed context. Each one gets the exact same Approve / Request Changes / regenerate-with-feedback flow already used everywhere else in this app (reused the same UI, not a new pattern), and the AI Movie sidebar now shows a real stage-progress list (green dot = approved, blue = the one you're on, grey = not reached yet) exactly like the Movie side's Idea/Synopsis/Characters/Bit Sheet list. New database column `stage_status` tracks each layer's state; old AI Movie projects saved before this (the real Akhada project included) load in cleanly — whatever they already had generated is treated as already-approved automatically, so nothing about existing work gets thrown away or re-asked-for. Generate-from-Reference now only originates the Story layer (used to generate the whole chain at once) so it feeds into this same step-by-step flow like everything else. New endpoints: generate one stage, approve one stage (which also silently refreshes the asset-extraction list with whatever's newly locked in). Verified extremely thoroughly given the size of this: every new endpoint's error paths (invalid stage, missing project, project not found) via curl; that Analyze+Proceed and Generate-from-Reference correctly compute which prefix of stages counts as already-approved; that an old-shape project with no `stage_status` at all correctly gets its existing content auto-approved on load; and the full real browser flow for every stage type (Synopsis, Plot, Character Arc, and the new Screenplay, which renders as real scene heading/action/dialogue) — generate, approve, and the sidebar/main-content updating correctly at each step, ending in a genuine "everything locked in" state with all 5 stages approved. Caught and fixed a real crash along the way: the Request Changes feedback box used the app's shared `MicTextarea` component, which needs a React context the AI Movie screen doesn't provide (deliberately, same reason its other text boxes are plain `<textarea>`) — it crashed the whole panel the moment Request Changes was clicked; fixed by using a plain textarea there too. Not verified: actual generation quality against a real Gemini key. Next: wait for the user's next instruction.
- Fixed the "manual SQL after every schema change" problem for good, per the user's direct instruction ("fix it permanently, it shouldn't come again and again") after the third time this session a missing AI Movie table/column (`ai_movie_projects`, `ai_movie_reference_files`, then `stage_status`) needed hand-run SQL in the Supabase dashboard. Root cause was never the SQL itself — it's that this sandbox has no network access to the live Supabase database, so a schema change made in code here had no way to reach production except by asking the user to run it by hand. Fixed by making the backend self-heal its own schema: on every server start (which Render already does automatically on every deploy to `main`), it now runs the AI Movie tables' `CREATE TABLE IF NOT EXISTS` / `ADD COLUMN IF NOT EXISTS` statements itself, safe to run every single time since none of them touch or remove existing data. From here on, an AI Movie schema change lands on the live database the moment the deploy finishes — no manual Supabase step, ever again, for this table going forward. Verified against two real scenarios on a local Postgres database (not the live Supabase, which this sandbox still can't reach): (1) both AI Movie tables dropped entirely → restarted the server → both came back with every column/type/default/foreign-key correct; (2) the more realistic case matching what actually happened in production — inserted a real data row, then dropped just the `stage_status` column → restarted the server → confirmed via `psql` that the column came back correctly AND the existing row's data was completely untouched. Can't verify this against the actual live Supabase database from this sandbox, only this equivalent local test, but the mechanism itself (idempotent, additive-only SQL run on every startup) doesn't depend on which database it's pointed at. Next: wait for the user's next instruction — this class of error (a missing AI Movie table or column in production) should no longer come up.
- Reworked the AI Movie stage order to match real screenwriting order and added the three pieces the user asked for by name, after confirming the plan with them first: (1) **Reordered the pipeline** from Story → Synopsis → Plot → Character Arc → Screenplay to **Story → Synopsis → Characters → Three-Act Structure → Beat Sheet → Screenplay** — Characters now comes before the plotting stages (cast the wants/needs before building beats around them), matching both real craft convention and the shooting pipeline's own Idea → Synopsis → Characters → Bit Sheet order. (2) **Added a brand-new Three-Act Structure stage** (the user noticed it was missing) — a new schema (act name, description, turning point per act) sitting between Characters and Beat Sheet as the macro shape the detailed beats get organized inside; "Plot" was renamed to **Beat Sheet** in the UI (it already was a beat list, just needed the right name) — no database migration needed, both slot into the existing JSONB `backfill`/`stage_status` columns. Screenplay/Dialogue stay parked exactly as built, per the user's own "up to bitset, then we design the screenplay part together." (3) **Added a real Delete button** to AI Movie's All Projects list (there wasn't one before) — new `DELETE /api/ai-movie/projects/:id` endpoint, cascades to that project's reference files automatically via the existing foreign key, same confirm-dialog pattern as the Movie side's own project delete. (4) **Added a "New Story: Akhada" button** that creates a fresh AI Movie project named "Akhada" and attaches the user's own uploaded story-bible files (the task + full 46-beat plot/characters/world lore, the scenic breakdown of those 46 beats with full visual description, and the two extra project rules) as Reference Material, auto-categorized the same way any upload already is — the files themselves now live in the repo at `app/backend/reference-material/akhada/` (read straight off disk, nothing invented) rather than needing the user to re-upload them by hand. Verified thoroughly against a local database and a real browser pass: every new/changed endpoint's error paths via curl (delete missing project → 404, generate/approve an invalid stage → 400, approve the new `threeAct` stage → succeeds), the reordered backfill-needed-layers logic reaching Gemini cleanly for the new stage combinations (this sandbox's dummy key gives the same expected clean auth error as every other agent here), and a full Playwright pass — clicked "New Story: Akhada," confirmed all 3 files attached with correct labels and their full content intact (not truncated) in the database, confirmed the project shows up in All Projects, clicked Delete, confirmed the confirm-dialog fires and the project disappears from the list afterward, zero console/page errors throughout. Can't verify this against the real live Akhada project from this sandbox (no network access to production) — next step is for the user to try "New Story: Akhada" on the live site themselves once this deploys, then walk Synopsis → Characters → Three-Act → Beat Sheet with the real Gemini key, generating and approving one stage at a time. Next: wait for the user's next instruction.
- User tried the new step-by-step pipeline live and asked a fair question: since the Akhada beat sheet, characters, and act structure are already fully written in the files handed off, why click Generate+Approve four separate times for content that was never going to change? Answer given and then built: every Generate call already treats attached Reference Material as authoritative grounding, so it wasn't going to invent conflicting content — but for a project where everything is already finished, four click-throughs for identical content is real friction with no benefit. Built a one-click shortcut specifically for Akhada: the Synopsis, all 6 character arcs (5 heroes + Kalapahada), the 5-part Three-Act Structure, and all 46 Beat Sheet entries were transcribed in English directly from the user's own uploaded files into a new `app/backend/reference-material/akhada/fixed-stage-content.js` — nothing invented, a faithful restatement of material that already exists. A new endpoint (`POST /api/ai-movie/projects/:id/fill-akhada-stages`) hands that fixed English content to Gemini for translation ONLY (explicit system instruction: never alter, invent, or edit the English, only add faithful Odia/Hindi) and auto-approves all four stages at once, landing the project ready for Screenplay with nothing to re-review. Split the 46-beat translation into two batches of 23 (learned from the earlier "Hajiro" large-output truncation bug this session) rather than risking one oversized call. A new "Fill Synopsis → Beat Sheet from your files (skip re-review)" button appears once Story exists on a project titled "Akhada" specifically — a deliberately narrow, purpose-built shortcut for this one project's already-finished material, not a generic feature. Verified thoroughly: confirmed the endpoint's guard (no Story yet → clean 400, not a crash) and its 404 for a missing project via curl; confirmed it reaches Gemini cleanly with a real payload once Story exists (this sandbox's dummy key gives the same expected clean auth error as every other agent here); and a full Playwright pass confirmed the button stays hidden until Story exists, appears correctly once it does (only for an "Akhada"-titled project), and clicking it fires the real request. Not verified: actual translation quality/success against a real Gemini key. Next: have the user click the new "Fill Synopsis → Beat Sheet" button on the real Akhada project (after Story is generated) and confirm all four stages land in one shot, in real Odia and Hindi, correctly reflecting their own material.
- The "Fill Synopsis → Beat Sheet" shortcut above hit exactly the bug it was trying to avoid: the user clicked it live and got "Something went wrong" with the button reverting to normal — the real cause was the endpoint holding one HTTP request open across 5 sequential Gemini calls (synopsis, characters, three-act, and the 46 beats in two batches), which reliably ran long enough to hit a network-level timeout before any of them could finish, same failure shape as the earlier oversized-reference-material bug but caused by call COUNT this time, not text size. Fixed by making the endpoint respond in milliseconds (confirmed: 14ms locally) and do the actual work afterward in the background, saving each stage to the database the moment it's translated (so a later failure doesn't lose earlier progress) and tracking running/done/error in memory; the frontend now polls a new `fill-akhada-status` endpoint every 4 seconds instead of waiting on one fragile response, then reloads the project once done. Verified thoroughly: confirmed the POST returns instantly via `time curl`, confirmed status correctly reports running immediately after and error (with the real underlying message, not a generic one) once the background job fails against this sandbox's dummy key, confirmed a stage saved before a failure stays saved, and a full Playwright pass confirmed the button shows "Filling in…" right after click and the real error surfaces through polling rather than the old immediate "Something went wrong." Next: have the user click "Fill Synopsis → Beat Sheet" again on the real Akhada project — this should no longer time out regardless of how long the 5 translation calls take underneath.
- The stage-fill fix above worked — the user got all the way to a real Screenplay generation on the live Akhada project, and found the next real problem: Screenplay only produced 3 scenes total, when the task is to expand the 46-beat Beat Sheet into roughly 120-150 full scenes ("a beat is a pocket, not one scene"). Root cause confirmed by reading the code: Screenplay generation was one single Gemini call for the ENTIRE film with a 16K-token budget — nowhere close to enough for a full bilingual screenplay, so it just wrote whatever fit and stopped. This was explicitly the "screenplay part" flagged earlier as needing its own design conversation rather than a guess — confirmed the rebuild approach with the user (one beat at a time, matching the exact pattern already proven on the shooting pipeline's "Write This Scene" feature) before building. Rebuilt Screenplay generation to write ONE beat's 2-4 scenes per Gemini call, 46 calls total, each one handed every scene already written so far as context so the draft reads as continuous rather than 46 disconnected fragments — and, learning directly from the stage-fill timeout just fixed, built it as a background job from the start rather than holding one request open across all 46 calls: the endpoint responds in ~20ms, and a new `GET /api/ai-movie/stages/screenplay/status` endpoint reports live progress ("beat 12 of 46") that the frontend polls every 4 seconds, showing a real progress line under the Screenplay heading instead of one static "Generating…". Every beat's scenes are saved to the database as soon as they're written (not just at the end), so a failure partway through doesn't lose earlier progress, and the whole draft still lands as "pending" for one real Approve/Request-Changes review at the end (unlike the earlier four-stage shortcut, screenplay dialogue genuinely needs a human read, so this one stays a real review gate). Caught and fixed a real bug while testing locally: the background status Map used the project id as its key, but the id arrived as a number from the POST body and a string from the GET query string — two different Map keys for the same project, so the very first status poll always read "none" even while the job was genuinely running; fixed by normalizing to a string. Verified thoroughly: confirmed the guard (no Beat Sheet yet → clean 400) via curl; confirmed the POST now returns in ~17-25ms regardless of beat count (`time curl`); confirmed status correctly reports `running` with the real total immediately after kickoff (not a race) and the real underlying error (not a generic one) once the background job fails against this sandbox's dummy key; confirmed a 2-beat seeded run showed correct `completed`/`total` counts; and a full Playwright pass confirmed the button, project state, and error surfacing all behave correctly end-to-end. Not verified: actual scene quality or true beat-to-beat continuity against a real Gemini key, and total runtime for a genuine 46-beat run (expect several minutes). Next: have the user click "Generate Screenplay" again on the real Akhada project (Beat Sheet already filled) and watch the new progress line — this is the first version built to actually finish a full-length draft rather than stopping after a handful of scenes.
- Big UI/UX pass on AI Movie, driven by real feedback after actually using the last build: (1) **Hindi instead of Odia everywhere in AI Movie.** The Odia output was Romanized/phonetic, not real script — Hindi was coming back usable — so every AI Movie field (Story, Synopsis, Characters, Three-Act, Beat Sheet, Screenplay) now asks for English + Hindi only. Built as its own schema (`AI_MOVIE_BILINGUAL_TEXT_SCHEMA`, en/hi) completely separate from the shared `BILINGUAL_TEXT_SCHEMA` (en/or/hi) the Movie/shooting side still uses untouched — same for the language dropdown, which now has its own `aiMovieLanguage` state instead of sharing the Movie side's toggle. (2) **Screenplay no longer writes dialogue** — just scene heading + action/visual description, matching the user's own original Scenic Breakdown file, which deliberately separated the two. Dialogue becomes a distinct later pass, designed separately, once the scene structure itself is right. (3) **Collapsible sections for every stage** (Story, Synopsis, Characters, Three-Act, Beat Sheet) — each auto-collapses into a single clickable "▸ Synopsis ✅ Approved" header line the moment it's approved, click to expand back open; whatever's still being worked on stays open. Fixes the "scrolling forever past everything I already finished" complaint directly. (4) **Screenplay rebuilt as a one-beat-at-a-time carousel with a 4-beat generation buffer**, replacing the flat "whole draft lands as one pending block" design from a few steps back — precisely spec'd by the user across several messages (including in Hindi) before building: `backfill.screenplayBeats` is now an array parallel to the Beat Sheet, each entry tracking its own `not_started/generating/pending/approved/error` status; only ONE beat's scenes show on screen at a time in a focused card; approving it collapses it into a compact "✅ Beat 2: [title] — approved" row and reveals the next ready beat; the backend keeps up to 4 beats generated ahead in the background at all times, topping the buffer back up by one every time a beat gets approved, so there's never a wait for the next one to write itself; a small note shows how many more are already sitting ready. New endpoints: `POST .../beats/:index/approve` and `POST .../beats/:index/request-changes` (the latter doubles as a plain retry when a beat's status comes back `error`, since it works identically with no feedback text). No new status-polling endpoint needed this time — since progress now lives entirely inside the project's own `backfill` (not an in-memory map), the frontend just polls the ordinary project-detail endpoint, which also sidesteps the exact number/string key-mismatch bug hit in the last build. Verified extremely thoroughly given the size of this change: confirmed the new POST still returns near-instantly regardless of beat count; confirmed the buffer-fill loop correctly stops after either an error or reaching the target size (not a runaway loop); manually drove a 3-beat project through approve → buffer-refill → approve → buffer-refill → final approve and confirmed the whole-stage `stage_status.screenplay` only flips to approved once every beat is; confirmed invalid-beat-index and missing-project guards on every new endpoint; and two full Playwright passes confirmed the Hindi-only dropdown, the collapsed-by-default stage headers (with working expand-on-click), and the screenplay carousel's exact visual shape (one approved row collapsed on top, the current beat's scenes with no dialogue, Approve/Request Changes, and an accurate "N more beats ready ahead" count that correctly excludes a beat still `generating`). Not verified: actual generation/translation quality against a real Gemini key. Next: have the user try the whole rebuilt flow on the real Akhada project — Hindi output, dialogue-free scenes, collapsible stages, and the new beat-by-beat screenplay carousel.
- Two more AI Movie layout requests: (1) the "Paste anything" concept box (intro text, textarea, Analyze/Proceed) now hides itself once a project actually has a Story — it was only ever needed for the very first input, and stayed sitting at the top forever after, ahead of the real working area. Now the main view goes straight to the stage cards once there's real content. (2) The whole "Reference Material" section (paste box, file upload, attached-file list, Generate Story from Reference Material) moved out of the main editor view entirely into its own screen, reached by a new "Reference Material" entry in the left sidebar (right under All Projects, same click-to-switch-view pattern already used everywhere else in this app) — it only appears once a project exists. Matches the user's own spec exactly: nothing to look past in the main working area once a project is underway, but reference files are still one click away whenever needed. Verified with Playwright: the Analyze button is visible on a fresh project (no story yet) and disappears the moment a Story exists; the Reference Material paste box is completely absent from the main editor view once a project has a story, and reappears correctly (with the existing attached-file list intact) on the new dedicated "Reference Material" screen reached from the sidebar. Next: wait for the user's next instruction.
- User asked directly whether the silent asset-extraction agent (never a visible chat bubble — reads everything gathered so far and quietly keeps Characters/Properties/Environments lists updated, built several steps back) was still running after all the recent AI Movie rework. Checked the actual code rather than assuming: it's still wired into Proceed, Generate Story from Reference Material, and approving Synopsis/Characters/Three-Act/Beat Sheet — but the Screenplay rebuild into the per-beat carousel a few steps back never wired it into the new per-beat approve endpoint, so it had gone silent specifically for Screenplay completions, exactly where new props/characters/locations are most likely to first appear. Fixed by refreshing the asset list once every screenplay beat is approved (matching how every other stage refreshes on ITS completion, not on every intermediate step). Verified locally: approved a single-beat screenplay project and confirmed in the logs that the refresh call fired and failed cleanly against this sandbox's dummy key (not a crash), with the approval itself still succeeding either way — same best-effort pattern already used everywhere else this agent runs. Next: wait for the user's next instruction.
- Four more fixes on the Screenplay carousel and Reference Material, all confirmed with the user first (including a round where they corrected their own earlier "2-4 scenes" spec down to "no fixed range at all"). (1) **Scene count per beat was unreliable** — the prompt only asked for 2-4 scenes in words and Gemini often gave just 1. Found the Gemini SDK's array schema actually supports a hard `minItems` constraint (not just a prompt hint), so beats now have a structural floor of 2 scenes with no ceiling at all — the user was explicit a beat should get as many scenes as it actually needs (2, 7, whatever), never forced toward a fixed target; the prompt was rewritten to say exactly that. Bumped the per-beat token budget up (8192 → 16384) to give a genuinely large beat room to breathe. (2) **Approved beats were a dead end** — collapsing into a small row with no way to reopen them. (3) **"N more beats ready ahead" was just a sentence, not real navigation.** Fixed (2) and (3) together by replacing the whole approach: the screenplay carousel is now real Prev/Next navigation across every beat, approved or not — clicking a beat's own card shows its content and, if approved, its badge; there's no separate "locked" collapsed state anymore, since navigating back to any beat just shows it again, full content and all. Approving a beat still auto-advances the view forward by one (keeping the "approve and move on" flow), but Prev/Next now let the user freely browse anywhere in the sequence, including beats still sitting in the generation buffer that haven't been reached yet. (4) **Reference Material was a one-way trip** — clicking it had no way back except All Projects → reopen the project. Fixed by making the sidebar button toggle: click once to open, click again to return to exactly the project screen you were on. Verified thoroughly: confirmed the new schema constraint doesn't break request construction (same clean auth-error wiring check as every other stage, now with the `minItems` field included); and a full Playwright pass on a seeded 3-beat project confirmed loading a project resumes on the first unfinished beat, Prev correctly shows an already-approved beat's full content with its badge (not a locked row), Next correctly reaches a not-yet-approved-but-already-generated beat two steps ahead without needing to approve the one in between, and the Reference Material button opens then closes back to the exact same project view on a second click. Not verified: actual scene-count behavior against a real Gemini key (this sandbox has none) — the fix changes what the model is asked and structurally constrained to do, but real output still needs the user's own check. Next: have the user regenerate a beat on the real Akhada project and confirm it comes back with a sensible number of scenes (not just 1), and try the new Prev/Next navigation and the Reference Material toggle.
- Made Screenplay generation runtime-aware, per the user's explicit instruction to make each beat's scene count follow from how much screen time it actually needs, using the standard "1 page ≈ 1 minute" screenwriting rule of thumb — rather than picking a scene count in isolation. First checked whether the exact per-beat minutes already existed in the source material: only an aggregate total plus qualitative "heavy beat" flags did, so an estimate was started — the user then stopped that and supplied their own exact, precise per-beat durations for all 46 beats instead (`Akhada_Beat_Sheet_46.md`, one row per beat with a running total), which were used in place of the estimate. Every Beat Sheet beat now carries a `runtimeMinutes` target (for Akhada, the user's own exact figures — swapped in via a verified positional replace, confirmed to sum to exactly 136.0 minutes matching the source's own stated total; for any other/future project, the AI estimates it honestly per-beat as part of normal Beat Sheet generation, told explicitly to vary beat length by its own dramatic weight rather than making every beat the same size). Every Screenplay scene now also carries its own `estimatedMinutes`. The Screenplay generation prompt was rewritten to state each beat's target minutes explicitly (translated into an implied page count) and let scene count/length follow from that — with the model told to keep its scenes' estimates honest and close to the target, not padded or compressed to force an exact match — and it falls back cleanly to the old need-driven-only behavior for any beat generated before this field existed (older projects, or non-Akhada beats without a target). The Akhada translation pass was also updated to explicitly copy `runtimeMinutes` straight through untouched (it's a number, not text to translate) rather than risk Gemini altering it. On the frontend: the Beat Sheet card and each Screenplay scene now show a "(~X min)" badge, and the Screenplay carousel shows a running total against the beat's own target (reusing the exact same `RuntimeSummary` component and wording already proven on the Movie/shooting side's Scene One-Liners) — no new CSS needed, matched the existing pattern instead of inventing one. Verified thoroughly: `node --check` on the backend; a seeded local project confirmed the new prompt reaches Gemini correctly (same clean, expected "invalid API key" error as every other agent here, meaning the request itself is well-formed); a full Playwright pass confirmed the Beat Sheet badge and the Screenplay running-total/per-scene badges all render correctly with real seeded `runtimeMinutes`/`estimatedMinutes` data and zero console errors. Not verified: actual generation quality/adherence to the runtime targets against a real Gemini key. Next: have the user regenerate a beat on the real Akhada project and confirm the scene count and estimated minutes actually track its target runtime sensibly.
- Gave the user direct control to lengthen any individual screenplay scene, right after the runtime-awareness feature above — they asked explicitly: if a scene the AI estimated at "1 min" needs more room, they should be free to extend it, and it's fine for the overall story/screenplay to grow past its original target as a result. Added a small "Extend this scene" button under every scene in the Screenplay carousel (on any beat, approved or still pending) that opens a tiny inline box for an optional instruction (e.g. "slow this down, let the moment breathe") — leaving it blank just asks for more screen time in general. New backend endpoint (`POST /api/ai-movie/stages/screenplay/beats/:beatIndex/scenes/:sceneIndex/extend`) rewrites ONLY that one scene — every other scene in the beat, and everything written in earlier beats, is handed to the AI as fixed, unchangeable context — and is allowed to split the extension into two or more scenes if the added material genuinely earns a real scene break (a location/time/focus change), rather than always forcing one long scene. Whichever beat the extended scene belongs to has its own `runtimeMinutes` target automatically recalculated to match the new, longer total — so the story is allowed to really grow, permanently, instead of the extension just being flagged afterward as "off from target." Also updated the main Screenplay-generation instructions so this same "the target is a floor, not a ceiling" rule applies to the existing whole-beat "Request Changes" flow too, if the user's own feedback there explicitly asks for more length. The beat goes back to "pending" after an extension (still one human look before it's final again), same as every other AI rewrite in this pipeline. Verified thoroughly: `node --check` on the backend; curl-tested every guard on the new endpoint (invalid beat, invalid scene, a beat with no scenes yet, missing project) and confirmed each fails cleanly with a sensible message rather than crashing; confirmed the endpoint reaches Gemini correctly (same clean, expected "invalid API key" error as every other agent here); and a full Playwright pass confirmed the "Extend this scene" button appears under every scene, the inline instruction box opens/cancels correctly, submitting fires the real request, and a failed request surfaces its real error in the UI while leaving the form open (nothing lost) rather than crashing the page. Not verified: actual output quality — whether the extended scene reads well and the split-vs-single-scene judgment is sensible — against a real Gemini key. Next: have the user try "Extend this scene" on a real scene in the live Akhada project and confirm both the writing and the updated running total feel right.
- User said directly "English + Hindi, not Odia" — checked the whole AI Movie section for any remaining place still asking for Odia rather than assuming the earlier Hindi-only switch had already caught everything, and found one real leftover: the silent Characters/Properties/Environments asset-extraction agent (the one that quietly keeps a reference-image-prep list updated on every stage's approval) was still built on the shared `BILINGUAL_TEXT_SCHEMA` (English/Odia/Hindi) instead of AI Movie's own `AI_MOVIE_BILINGUAL_TEXT_SCHEMA` (English/Hindi only) — missed during the earlier sweep since it's a background agent with no visible chat bubble, easy to overlook. Fixed by pointing it at the correct AI Movie-only schema, same as every other AI Movie field. Also hardened the AI Movie screen itself so it can never show Odia UI text (button labels, headings) even by accident: the general Movie-side language dropdown (English/Odia) and AI Movie's own content-language dropdown (English/Hindi, `aiMovieLanguage`) are two separate, independent settings, and AI Movie's screen was reading its labels from whichever one the general dropdown happened to be set to — harmless today only because there's no way back to the Movie/AI-Movie picker without a full page reload (which resets both settings together), but a real structural gap that a future "switch mode" button could easily reopen. Pinned AI Movie's own chrome to always read English labels, regardless of the general dropdown, so this can't happen no matter what gets built around it later. Verified: `node --check` on the backend; confirmed the asset-extraction endpoint still reaches Gemini cleanly (same clean, expected "invalid API key" error, meaning the corrected schema is well-formed); confirmed live in the browser that AI Movie's sidebar and stage labels still render correctly in English with zero regressions. Next: wait for the user's next instruction.
- User asked for each screenplay scene's duration to be clearly shown and left-aligned, not centered. It was already being shown, but tucked inline as a parenthetical right after the scene heading (e.g. "INT. RECEIVER STATION - DAY (~1.5 min)") — not its own clearly-labeled line, which is likely what read as unclear/oddly-positioned. Gave it a proper standalone line under each scene heading, reading "Duration: ~1.5 min", and added explicit `text-align: left` to the scene heading, the duration line, the scene row wrapper, and the beat's own running-total line — defensive belt-and-braces, since nothing in the actual CSS was found centering any of this, but it costs nothing to make left-alignment explicit rather than relying on it just happening to be the default. Verified live in the browser with a seeded project: each scene now shows its heading, then a separate "Duration: ~X min" line, then its action text, all left-aligned. Next: wait for the user's next instruction.
- User then sent real screenshots from the live Akhada project showing NO duration stamp at all on real Beat Sheet beats and Screenplay scenes — the actual root cause: those beats/scenes were written before the runtimeMinutes/estimatedMinutes fields existed, so they genuinely have no stored duration, and the display code was correctly showing nothing for a field that isn't there (the documented fallback from a few steps back, now shown to actually bite on real, already-in-progress work rather than just old/hypothetical data). Rather than leave already-written beats and scenes permanently blank, added a deterministic fallback estimate for anything missing a real one: a word-count-based guess using the same "~1 page/minute" rule of thumb (roughly 180 words per minute of screen time), with a 0.5-minute floor so nothing shows as 0. A beat with real written scenes now estimates its own duration from THEIR total (real content, not a guess about the beat description) — using their own real estimatedMinutes where scenes have it, falling back to the same word-count guess only for scenes that don't; a beat with no scenes yet falls back to estimating from its own description text. This runs live in the browser on every render (nothing gets silently rewritten in the database) so it self-heals immediately for every already-existing beat and scene, no migration needed, while freshly-generated content keeps using the AI's own real, context-aware estimate exactly as before. Verified live with a seeded project built to match the user's exact screenshots (beats/scenes stripped of the duration fields entirely) — every beat and scene now shows a duration, and beats with real scenes show a total derived from those scenes rather than a raw guess. Next: wait for the user's next instruction — the live Akhada project should now show a duration everywhere, without needing anything regenerated.
- Added "resume from where we left off" for AI Movie, matching a pattern the Movie/shooting side already had. Two real gaps closed together: (1) the Screenplay carousel itself already resumed at the first unfinished beat when a project was loaded (built a few steps back) — that part was already working. (2) What was actually missing: getting to that project at all required re-clicking through All Projects every single time, on every reload or fresh login, because (unlike the Movie side's own last-project memory) nothing about the currently-open AI Movie project was ever remembered between visits. Fixed by saving the current AI Movie project's id to the browser's own local storage every time one loads or gets created (via Analyze, Generate-from-Reference, or attaching Reference Material), and clearing it on "New Idea" or when the currently-open project gets deleted — same lifecycle as the Movie side's own memory. Clicking "AI Movie" on the Movie/AI-Movie picker now checks for a remembered project and, if one exists, loads it directly instead of landing on a blank paste box — taking you straight back to the exact project AND the exact Screenplay beat you were last working on, in one click, instead of Picker → All Projects → find it → click it every time. The picker screen itself still always appears after a reload/fresh login on purpose (an intentional, existing "easy way out" if you want to switch to the Movie side instead) — only what happens after choosing "AI Movie" changed. A remembered project that's since been deleted (or doesn't exist in this environment) fails gracefully: shows the normal error message and forgets that stale id rather than getting stuck trying it again on every future visit. Verified live end-to-end: loaded a seeded 3-beat project (approved beat 1, pending beat 2), did a full page reload, and confirmed clicking "AI Movie" landed directly back on Beat 2 of 3 with no extra clicks; separately confirmed "New Idea" correctly forgets the remembered project (a reload afterward returns to the blank editor, not the old project). Next: wait for the user's next instruction.
- User re-sent the exact same 46-beat duration file from earlier and asked to actually apply it. The real numbers were already sitting correctly in the codebase (`AKHADA_FIXED_BEATS`, set several steps back, verified summing to exactly 136.0) — the real gap was that the LIVE Akhada project's already-saved Beat Sheet was filled in before that field existed, so it has no duration on any beat at all (exactly what the last two steps' screenshots showed). Since this sandbox has no network access to the live production database, built a one-click way for the user to apply it themselves rather than trying to patch production directly: a new "Sync exact beat durations from your files" button (appears on an Akhada-titled project once its Beat Sheet exists) that calls a new, purely numeric endpoint (`POST /api/ai-movie/projects/:id/fill-akhada-beat-durations`, no Gemini call, instant) which matches each saved beat against `AKHADA_FIXED_BEATS` by position and sets ONLY its `runtimeMinutes` — title, description, any already-written screenplay scenes, and every other stage are left completely untouched. Includes a safety check: a beat whose title doesn't closely match what's expected at that position (translation drift, or a Beat Sheet since edited/regenerated) is skipped rather than getting the wrong number silently, and the button's result note reports exactly how many of the 46 beats were actually updated so any mismatch is visible, not hidden. Verified thoroughly against a seeded local project built to match the real gap exactly (Beat Sheet titles matching the fixed content, no runtimeMinutes on any beat): confirmed via curl that a deliberately mismatched title is correctly skipped and reported while the other matching beats update correctly with the exact right numbers (2.5, 2, 1.5, 3 — matching the file exactly); confirmed live in the browser that the button appears in the right place, clicking it applies the fix, and the result note ("Updated 5 of 5 beats...") displays correctly. Next: **this one needs the user's own click** — this sandbox cannot reach the live Supabase database, so open the real Akhada project, expand Beat Sheet, and click "Sync exact beat durations from your files" once to apply the real numbers there; after that, the Screenplay stage's own generation will also start using the correct exact target for every beat, not just the display.
